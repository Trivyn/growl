            ...
            printf("%s\n", " triples, indexing...");
            {
                ...
                printf("%s\n", " triples, reasoning...");
                {
                    ...
                    switch (_mv_340.tag) {
                        ...
                        case types_ReasonerResult_reason_inconsistent:
                        {
                            __auto_type report = _mv_340.data.reason_inconsistent;
...
uint8_t test_cli_test_invalid_literal_file(slop_arena* arena) {
    __auto_type _mv_341 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/invalid-literal.ttl"));
...
uint8_t test_cli_test_valid_literals_file(slop_arena* arena) {
    __auto_type _mv_343 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/valid-literals.ttl"));
    if (!_mv_343.has_value) {
        ...
        switch (_mv_344.tag) {
            ...
            case types_ReasonerResult_reason_inconsistent:
            {
                __auto_type report = _mv_344.data.reason_inconsistent;
...
uint8_t test_cli_test_emit_roundtrip(slop_arena* arena) {
    __auto_type _mv_345 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/subclass-chain.ttl"));
        ...
        switch (_mv_346.tag) {
            case types_ReasonerResult_reason_success:
            {
                __auto_type s = _mv_346.data.reason_success;
                {
                    ...
                    __auto_type _mv_347 = serialize_ttl_serialize_ttl_stream(arena, out_graph, config, emit_path);
                    if (!_mv_347.is_ok) {
                        __auto_type _ = _mv_347.data.err;
                        printf("%s\n", "  ERROR: failed to write roundtrip file");
                        return 0;
                    } else if (_mv_347.is_ok) {
                        __auto_type _ = _mv_347.data.ok;
                        __auto_type _mv_348 = ttl_parse_ttl_file(arena, emit_path);
                        if (!_mv_348.is_ok) {
                            __auto_type _ = _mv_348.data.err;
                            printf("%s\n", "  ERROR: failed to re-parse emitted file");
                            return 0;
                        } else if (_mv_348.is_ok) {
                            __auto_type g2 = _mv_348.data.ok;
                            {
...
uint8_t test_cli_test_missing_file(slop_arena* arena) {
    __auto_type _mv_349 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/does-not-exist.ttl"));
    if (!_mv_349.has_value) {
        return 1;
    } else if (_mv_349.has_value) {
        __auto_type _ = _mv_349.value;
