...
slop_option_types_ReasonerResult test_cli_parse_and_reason(slop_arena* arena, slop_string path) {
    __auto_type _mv_230 = ttl_parse_ttl_file(arena, path);
    if (!_mv_230.is_ok) {
        __auto_type _ = _mv_230.data.err;
        return (slop_option_types_ReasonerResult){.has_value = false};
    } else if (_mv_230.is_ok) {
        __auto_type g = _mv_230.data.ok;
        {
...
uint8_t test_cli_test_subclass_chain_file(slop_arena* arena) {
    __auto_type _mv_231 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/subclass-chain.ttl"));
    if (!_mv_231.has_value) {
        printf("%s\n", "  ERROR: failed to parse subclass-chain.ttl");
        return 0;
    } else if (_mv_231.has_value) {
        __auto_type result = _mv_231.value;
        __auto_type _mv_232 = result;
        switch (_mv_232.tag) {
            case types_ReasonerResult_reason_success:
            {
                __auto_type s = _mv_232.data.reason_success;
                {
            ...
            {
                __auto_type _ = _mv_232.data.reason_inconsistent;
...
uint8_t test_cli_test_equivalent_class_file(slop_arena* arena) {
    __auto_type _mv_233 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/equivalent-class.ttl"));
    if (!_mv_233.has_value) {
        printf("%s\n", "  ERROR: failed to parse equivalent-class.ttl");
        return 0;
    } else if (_mv_233.has_value) {
        __auto_type result = _mv_233.value;
        __auto_type _mv_234 = result;
        switch (_mv_234.tag) {
            case types_ReasonerResult_reason_success:
            {
                __auto_type s = _mv_234.data.reason_success;
                {
                    __auto_type alice = rdf_make_iri(arena, SLOP_STR("http://example.org/alice"));
            ...
            case types_ReasonerResult_reason_inconsistent:
            {
                __auto_type _ = _mv_234.data.reason_inconsistent;
                printf("%s\n", "  ERROR: unexpected inconsistency");
                return 0;
            }
        }
...
uint8_t test_cli_test_disjoint_violation_file(slop_arena* arena) {
    __auto_type _mv_235 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/disjoint-violation.ttl"));
    if (!_mv_235.has_value) {
        printf("%s\n", "  ERROR: failed to parse disjoint-violation.ttl");
        return 0;
    } else if (_mv_235.has_value) {
        __auto_type result = _mv_235.value;
        __auto_type _mv_236 = result;
        switch (_mv_236.tag) {
            case types_ReasonerResult_reason_success:
            {
                __auto_type _ = _mv_236.data.reason_success;
                printf("%s\n", "  ERROR: should have detected inconsistency");
                return 0;
            }
            case types_ReasonerResult_reason_inconsistent:
            {
                __auto_type _ = _mv_236.data.reason_inconsistent;
                return 1;
            }
        }
    }
...
uint8_t test_cli_test_sameas_differentfrom_file(slop_arena* arena) {
    __auto_type _mv_237 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/sameas-differentfrom.ttl"));
    if (!_mv_237.has_value) {
        printf("%s\n", "  ERROR: failed to parse sameas-differentfrom.ttl");
        return 0;
    } else if (_mv_237.has_value) {
        __auto_type result = _mv_237.value;
        __auto_type _mv_238 = result;
        switch (_mv_238.tag) {
            case types_ReasonerResult_reason_success:
            {
                __auto_type _ = _mv_238.data.reason_success;
                printf("%s\n", "  ERROR: should have detected inconsistency");
                return 0;
            }
            case types_ReasonerResult_reason_inconsistent:
            {
                __auto_type _ = _mv_238.data.reason_inconsistent;
                return 1;
            }
        }
    }
...
uint8_t test_cli_test_sameas_chain_file(slop_arena* arena) {
    __auto_type _mv_239 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/sameas-chain.ttl"));
    if (!_mv_239.has_value) {
        printf("%s\n", "  ERROR: failed to parse sameas-chain.ttl");
        return 0;
    } else if (_mv_239.has_value) {
        __auto_type result = _mv_239.value;
        __auto_type _mv_240 = result;
        switch (_mv_240.tag) {
            case types_ReasonerResult_reason_success:
            {
                __auto_type s = _mv_240.data.reason_success;
                {
                    __auto_type alice = rdf_make_iri(arena, SLOP_STR("http://example.org/alice"));
            ...
            case types_ReasonerResult_reason_inconsistent:
            {
                __auto_type _ = _mv_240.data.reason_inconsistent;
                printf("%s\n", "  ERROR: unexpected inconsistency");
                return 0;
            }
        }
...
uint8_t test_cli_test_empty_file(slop_arena* arena) {
    __auto_type _mv_241 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/empty.ttl"));
    if (!_mv_241.has_value) {
        printf("%s\n", "  ERROR: failed to parse empty.ttl");
        return 0;
    } else if (_mv_241.has_value) {
        __auto_type result = _mv_241.value;
        __auto_type _mv_242 = result;
        switch (_mv_242.tag) {
            case types_ReasonerResult_reason_success:
            {
                __auto_type s = _mv_242.data.reason_success;
                return (s.inferred_count == 0);
            }
            case types_ReasonerResult_reason_inconsistent:
            {
                __auto_type _ = _mv_242.data.reason_inconsistent;
...
uint8_t test_cli_test_bfo_consistent(slop_arena* arena) {
    {
        ...
        printf("%s\n", "  BFO: parsing...");
        __auto_type _mv_243 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/bfo-core.ttl"));
        if (!_mv_243.is_ok) {
            __auto_type _ = _mv_243.data.err;
            printf("%s\n", "  ERROR: failed to parse bfo-core.ttl");
            return 0;
        } else if (_mv_243.is_ok) {
            __auto_type g = _mv_243.data.ok;
            printf("%s", "  BFO: parsed ");
            ...
            printf("%s\n", " triples, indexing...");
            {
                ...
                printf("%s\n", " triples, reasoning...");
                {
                    ...
                    __auto_type _mv_244 = growl_reason(arena, ig);
                    switch (_mv_244.tag) {
                        case types_ReasonerResult_reason_success:
                        {
                            __auto_type s = _mv_244.data.reason_success;
                            {
                        ...
                        case types_ReasonerResult_reason_inconsistent:
                        {
                            __auto_type report = _mv_244.data.reason_inconsistent;
...
uint8_t test_cli_test_emit_roundtrip(slop_arena* arena) {
    __auto_type _mv_245 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/subclass-chain.ttl"));
    if (!_mv_245.has_value) {
        printf("%s\n", "  ERROR: failed to parse for roundtrip");
        return 0;
    } else if (_mv_245.has_value) {
        __auto_type result = _mv_245.value;
        __auto_type _mv_246 = result;
        switch (_mv_246.tag) {
            case types_ReasonerResult_reason_success:
            {
                __auto_type s = _mv_246.data.reason_success;
                {
                    ...
                    __auto_type _mv_247 = serialize_ttl_serialize_ttl_stream(arena, out_graph, config, emit_path);
                    if (!_mv_247.is_ok) {
                        __auto_type _ = _mv_247.data.err;
                        printf("%s\n", "  ERROR: failed to write roundtrip file");
                        return 0;
                    } else if (_mv_247.is_ok) {
                        __auto_type _ = _mv_247.data.ok;
                        __auto_type _mv_248 = ttl_parse_ttl_file(arena, emit_path);
                        if (!_mv_248.is_ok) {
                            __auto_type _ = _mv_248.data.err;
                            printf("%s\n", "  ERROR: failed to re-parse emitted file");
                            return 0;
                        } else if (_mv_248.is_ok) {
                            __auto_type g2 = _mv_248.data.ok;
                            {
            ...
            case types_ReasonerResult_reason_inconsistent:
            {
                __auto_type _ = _mv_246.data.reason_inconsistent;
...
uint8_t test_cli_test_missing_file(slop_arena* arena) {
    __auto_type _mv_249 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/does-not-exist.ttl"));
    if (!_mv_249.has_value) {
        return 1;
    } else if (_mv_249.has_value) {
        __auto_type _ = _mv_249.value;
