...
uint8_t test_cli_test_subclass_chain_file(slop_arena* arena) {
    __auto_type _mv_337 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/subclass-chain.ttl"));
...
uint8_t test_cli_test_equivalent_class_file(slop_arena* arena) {
    __auto_type _mv_339 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/equivalent-class.ttl"));
...
uint8_t test_cli_test_disjoint_violation_file(slop_arena* arena) {
    __auto_type _mv_341 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/disjoint-violation.ttl"));
    if (!_mv_341.has_value) {
        printf("%s\n", "  ERROR: failed to parse disjoint-violation.ttl");
        return 0;
    } else if (_mv_341.has_value) {
        __auto_type result = _mv_341.value;
        __auto_type _mv_342 = result;
        switch (_mv_342.tag) {
            case types_ReasonerResult_reason_success:
            {
                __auto_type _ = _mv_342.data.reason_success;
                printf("%s\n", "  ERROR: should have detected inconsistency");
                return 0;
            }
            case types_ReasonerResult_reason_inconsistent:
            {
                __auto_type _ = _mv_342.data.reason_inconsistent;
                return 1;
            }
        }
    }
...
uint8_t test_cli_test_sameas_differentfrom_file(slop_arena* arena) {
    __auto_type _mv_343 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/sameas-differentfrom.ttl"));
...
uint8_t test_cli_test_sameas_chain_file(slop_arena* arena) {
    __auto_type _mv_345 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/sameas-chain.ttl"));
    if (!_mv_345.has_value) {
        ...
        switch (_mv_346.tag) {
            case types_ReasonerResult_reason_success:
            {
                __auto_type s = _mv_346.data.reason_success;
                {
                    __auto_type alice = rdf_make_iri(arena, SLOP_STR("http://example.org/alice"));
            ...
            case types_ReasonerResult_reason_inconsistent:
            {
                __auto_type _ = _mv_346.data.reason_inconsistent;
                printf("%s\n", "  ERROR: unexpected inconsistency");
                return 0;
            }
        }
...
uint8_t test_cli_test_empty_file(slop_arena* arena) {
    __auto_type _mv_347 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/empty.ttl"));
...
uint8_t test_cli_test_bfo_consistent(slop_arena* arena) {
    {
        ...
        printf("%s\n", "  BFO: parsing...");
        __auto_type _mv_349 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/bfo-core.ttl"));
        if (!_mv_349.is_ok) {
            __auto_type _ = _mv_349.data.err;
            printf("%s\n", "  ERROR: failed to parse bfo-core.ttl");
            return 0;
        } else if (_mv_349.is_ok) {
            __auto_type g = _mv_349.data.ok;
            printf("%s", "  BFO: parsed ");
            ...
            printf("%s\n", " triples, indexing...");
            {
                ...
                printf("%s\n", " triples, reasoning...");
                {
                    ...
                    __auto_type _mv_350 = growl_reason(arena, ig);
                    switch (_mv_350.tag) {
                        case types_ReasonerResult_reason_success:
                        {
                            __auto_type s = _mv_350.data.reason_success;
                            {
                        ...
                        case types_ReasonerResult_reason_inconsistent:
                        {
                            __auto_type reports = _mv_350.data.reason_inconsistent;
                            printf("%s", "  ERROR: BFO inconsistent: ");
                            __auto_type _mv_351 = ({ __auto_type _lst = reports; size_t _idx = (size_t)0; slop_option_types_InconsistencyReport _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_351.has_value) {
                                __auto_type report = _mv_351.value;
...
uint8_t test_cli_test_invalid_literal_file(slop_arena* arena) {
    __auto_type _mv_352 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/invalid-literal.ttl"));
    if (!_mv_352.has_value) {
        printf("%s\n", "  ERROR: failed to parse invalid-literal.ttl");
        return 0;
    } else if (_mv_352.has_value) {
        __auto_type result = _mv_352.value;
        __auto_type _mv_353 = result;
        switch (_mv_353.tag) {
            case types_ReasonerResult_reason_success:
            {
...
uint8_t test_cli_test_valid_literals_file(slop_arena* arena) {
    __auto_type _mv_354 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/valid-literals.ttl"));
    if (!_mv_354.has_value) {
        ...
        switch (_mv_355.tag) {
            ...
            case types_ReasonerResult_reason_inconsistent:
            {
                __auto_type reports = _mv_355.data.reason_inconsistent;
                printf("%s", "  ERROR: unexpected inconsistency: ");
                __auto_type _mv_356 = ({ __auto_type _lst = reports; size_t _idx = (size_t)0; slop_option_types_InconsistencyReport _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_356.has_value) {
                    __auto_type report = _mv_356.value;
                    printf("%.*s\n", (int)(report.reason).len, (report.reason).data);
        ...
        switch (_mv_358.tag) {
            case types_ReasonerResult_reason_success:
            {
                __auto_type s = _mv_358.data.reason_success;
                {
                    ...
                    __auto_type _mv_359 = serialize_ttl_serialize_ttl_stream(arena, out_graph, config, emit_path);
                    if (!_mv_359.is_ok) {
                        __auto_type _ = _mv_359.data.err;
                        printf("%s\n", "  ERROR: failed to write roundtrip file");
                        return 0;
                    } else if (_mv_359.is_ok) {
                        __auto_type _ = _mv_359.data.ok;
                        __auto_type _mv_360 = ttl_parse_ttl_file(arena, emit_path);
                        if (!_mv_360.is_ok) {
                            __auto_type _ = _mv_360.data.err;
                            printf("%s\n", "  ERROR: failed to re-parse emitted file");
                            return 0;
                        } else if (_mv_360.is_ok) {
                            __auto_type g2 = _mv_360.data.ok;
                            {
...
uint8_t test_cli_test_missing_file(slop_arena* arena) {
    __auto_type _mv_361 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/does-not-exist.ttl"));
...
uint8_t test_cli_test_validate_unsat(slop_arena* arena) {
    __auto_type _mv_362 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/validate-unsat.ttl"));
    if (!_mv_362.is_ok) {
        ...
        __auto_type g = _mv_362.data.ok;
        {
            ...
            switch (_mv_363.tag) {
                case types_ReasonerResult_reason_inconsistent:
                {
                    __auto_type _ = _mv_363.data.reason_inconsistent;
                    return 1;
                }
                case types_ReasonerResult_reason_success:
                {
...
uint8_t test_cli_test_validate_clean(slop_arena* arena) {
    __auto_type _mv_364 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/validate-clean.ttl"));
    if (!_mv_364.is_ok) {
        __auto_type _ = _mv_364.data.err;
        printf("%s\n", "  ERROR: failed to parse validate-clean.ttl");
        return 0;
    } else if (_mv_364.is_ok) {
        __auto_type g = _mv_364.data.ok;
        {
            __auto_type ig = test_cli_graph_to_indexed(arena, g);
            __auto_type config = ((types_ReasonerConfig){.worker_count = 4, .channel_buffer = 256, .max_iterations = 1000, .verbose = 0, .fast = 0, .complete = 0, .enrich = 0, .validate = 1, .validate_ns = SLOP_STR(""), .cancel_ptr = 0, .max_triples = 0});
            __auto_type _mv_365 = growl_reason_with_config(arena, ig, config);
            switch (_mv_365.tag) {
                case types_ReasonerResult_reason_success:
                {
                ...
                case types_ReasonerResult_reason_inconsistent:
                {
                    __auto_type reports = _mv_365.data.reason_inconsistent;
                    printf("%s", "  ERROR: unexpected inconsistency: ");
                    __auto_type _mv_366 = ({ __auto_type _lst = reports; size_t _idx = (size_t)0; slop_option_types_InconsistencyReport _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_366.has_value) {
                        __auto_type report = _mv_366.value;
                        printf("%.*s\n", (int)(report.reason).len, (report.reason).data);
...
uint8_t test_cli_test_validate_ns_scoping(slop_arena* arena) {
    __auto_type _mv_374 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/validate-bg-domain.ttl"));
    if (!_mv_374.is_ok) {
    ...
    if (!_mv_377.is_ok) {
        ...
        __auto_type g = _mv_377.data.ok;
        {
            ...
            switch (_mv_378.tag) {
                case types_ReasonerResult_reason_success:
                {
                    __auto_type _ = _mv_378.data.reason_success;
                    return 1;
                }
                case types_ReasonerResult_reason_inconsistent:
                {
                    __auto_type reports = _mv_378.data.reason_inconsistent;
                    printf("%s", "  ERROR: unexpected inconsistency: ");
                    __auto_type _mv_379 = ({ __auto_type _lst = reports; size_t _idx = (size_t)0; slop_option_types_InconsistencyReport _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_379.has_value) {
                        __auto_type report = _mv_379.value;
                        printf("%.*s\n", (int)(report.reason).len, (report.reason).data);
...
uint8_t test_cli_test_background_merge(slop_arena* arena) {
    __auto_type _mv_380 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/validate-bg-tlo.ttl"));
    if (!_mv_380.is_ok) {
        __auto_type _ = _mv_380.data.err;
        printf("%s\n", "  ERROR: failed to parse validate-bg-tlo.ttl");
        return 0;
    } else if (_mv_380.is_ok) {
        __auto_type bg_graph = _mv_380.data.ok;
        __auto_type _mv_381 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/validate-domain-only.ttl"));
        if (!_mv_381.is_ok) {
            __auto_type _ = _mv_381.data.err;
            printf("%s\n", "  ERROR: failed to parse validate-domain-only.ttl");
            return 0;
        } else if (_mv_381.is_ok) {
            __auto_type domain_graph = _mv_381.data.ok;
            {
                ...
                }
                {
                    ...
                    switch (_mv_382.tag) {
                        case types_ReasonerResult_reason_inconsistent:
                        {
                            ...
                            __auto_type _mv_383 = ({ __auto_type _lst = reports; size_t _idx = (size_t)0; slop_option_types_InconsistencyReport _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_383.has_value) {
                                __auto_type report = _mv_383.value;
                                return strlib_starts_with(report.reason, SLOP_STR("Unsatisfiable class:"));
...
uint8_t test_cli_test_validate_multiple_unsat(slop_arena* arena) {
    __auto_type _mv_384 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/validate-multi-unsat.ttl"));
    if (!_mv_384.is_ok) {
...
uint8_t test_cli_test_enrich_basic(slop_arena* arena) {
    __auto_type _mv_386 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/subclass-chain.ttl"));
    if (!_mv_386.is_ok) {
        __auto_type _ = _mv_386.data.err;
        printf("%s\n", "  ERROR: failed to parse subclass-chain.ttl");
        return 0;
    } else if (_mv_386.is_ok) {
        __auto_type g = _mv_386.data.ok;
        {
            __auto_type ig = test_cli_graph_to_indexed(arena, g);
            ...
            __auto_type _mv_387 = growl_reason_with_config(arena, ig, config);
            switch (_mv_387.tag) {
                case types_ReasonerResult_reason_success:
                {
                    __auto_type s = _mv_387.data.reason_success;
                    {
                        __auto_type alice = rdf_make_iri(arena, SLOP_STR("http://example.org/alice"));
                        __auto_type person = rdf_make_iri(arena, SLOP_STR("http://example.org/Person"));
                        __auto_type agent = rdf_make_iri(arena, SLOP_STR("http://example.org/Agent"));
                ...
                case types_ReasonerResult_reason_inconsistent:
                {
                    __auto_type _ = _mv_387.data.reason_inconsistent;
                    printf("%s\n", "  ERROR: unexpected inconsistency");
                    return 0;
                }
            }
...
uint8_t test_cli_test_enrich_skips_eq(slop_arena* arena) {
    __auto_type _mv_388 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/sameas-chain.ttl"));
    if (!_mv_388.is_ok) {
        __auto_type _ = _mv_388.data.err;
        printf("%s\n", "  ERROR: failed to parse sameas-chain.ttl");
        return 0;
    } else if (_mv_388.is_ok) {
        __auto_type g = _mv_388.data.ok;
        {
            __auto_type ig = test_cli_graph_to_indexed(arena, g);
            __auto_type config = ((types_ReasonerConfig){.worker_count = 4, .channel_buffer = 256, .max_iterations = 1000, .verbose = 0, .fast = 0, .complete = 0, .enrich = 1, .validate = 0, .validate_ns = SLOP_STR(""), .cancel_ptr = 0, .max_triples = 0});
            __auto_type _mv_389 = growl_reason_with_config(arena, ig, config);
            switch (_mv_389.tag) {
                case types_ReasonerResult_reason_success:
                {
                    __auto_type s = _mv_389.data.reason_success;
                    {
                        __auto_type alice = rdf_make_iri(arena, SLOP_STR("http://example.org/alice"));
                        __auto_type charlie = rdf_make_iri(arena, SLOP_STR("http://example.org/charlie"));
                        return !(test_cli_has_same_as(arena, s.graph, alice, charlie));
                ...
                case types_ReasonerResult_reason_inconsistent:
                {
                    __auto_type _ = _mv_389.data.reason_inconsistent;
                    printf("%s\n", "  ERROR: unexpected inconsistency");
                    return 0;
                }
            }
...
uint8_t test_cli_test_enrich_skips_cls(slop_arena* arena) {
    __auto_type _mv_390 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/enrich-cls-test.ttl"));
    if (!_mv_390.is_ok) {
        __auto_type _ = _mv_390.data.err;
        printf("%s\n", "  ERROR: failed to parse enrich-cls-test.ttl");
        return 0;
    } else if (_mv_390.is_ok) {
        __auto_type g = _mv_390.data.ok;
        {
            __auto_type ig = test_cli_graph_to_indexed(arena, g);
            __auto_type config = ((types_ReasonerConfig){.worker_count = 4, .channel_buffer = 256, .max_iterations = 1000, .verbose = 0, .fast = 0, .complete = 0, .enrich = 1, .validate = 0, .validate_ns = SLOP_STR(""), .cancel_ptr = 0, .max_triples = 0});
            __auto_type _mv_391 = growl_reason_with_config(arena, ig, config);
            switch (_mv_391.tag) {
                case types_ReasonerResult_reason_success:
                {
                    __auto_type s = _mv_391.data.reason_success;
                    {
                        __auto_type alice = rdf_make_iri(arena, SLOP_STR("http://example.org/alice"));
                ...
                case types_ReasonerResult_reason_inconsistent:
                {
                    __auto_type _ = _mv_391.data.reason_inconsistent;
                    printf("%s\n", "  ERROR: unexpected inconsistency");
                    return 0;
                }
            }
...
uint8_t test_cli_test_enrich_checks_consistency(slop_arena* arena) {
    __auto_type _mv_392 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/disjoint-violation.ttl"));
    if (!_mv_392.is_ok) {
        ...
        __auto_type g = _mv_392.data.ok;
        {
            __auto_type ig = test_cli_graph_to_indexed(arena, g);
            __auto_type config = ((types_ReasonerConfig){.worker_count = 4, .channel_buffer = 256, .max_iterations = 1000, .verbose = 0, .fast = 0, .complete = 0, .enrich = 1, .validate = 0, .validate_ns = SLOP_STR(""), .cancel_ptr = 0, .max_triples = 0});
            __auto_type _mv_393 = growl_reason_with_config(arena, ig, config);
            switch (_mv_393.tag) {
                case types_ReasonerResult_reason_inconsistent:
                {
                    __auto_type _ = _mv_393.data.reason_inconsistent;
                    return 1;
                }
                case types_ReasonerResult_reason_success:
                {
                    __auto_type _ = _mv_393.data.reason_success;
