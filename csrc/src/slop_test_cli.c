...
slop_option_types_ReasonerResult test_cli_parse_and_reason(slop_arena* arena, slop_string path) {
    __auto_type _mv_349 = ttl_parse_ttl_file(arena, path);
    if (!_mv_349.is_ok) {
...
uint8_t test_cli_test_subclass_chain_file(slop_arena* arena) {
    __auto_type _mv_350 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/subclass-chain.ttl"));
...
uint8_t test_cli_test_equivalent_class_file(slop_arena* arena) {
    __auto_type _mv_352 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/equivalent-class.ttl"));
...
uint8_t test_cli_test_disjoint_violation_file(slop_arena* arena) {
    __auto_type _mv_354 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/disjoint-violation.ttl"));
...
uint8_t test_cli_test_sameas_differentfrom_file(slop_arena* arena) {
    __auto_type _mv_356 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/sameas-differentfrom.ttl"));
...
uint8_t test_cli_test_sameas_chain_file(slop_arena* arena) {
    __auto_type _mv_358 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/sameas-chain.ttl"));
...
uint8_t test_cli_test_empty_file(slop_arena* arena) {
    __auto_type _mv_360 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/empty.ttl"));
    if (!_mv_360.has_value) {
        printf("%s\n", "  ERROR: failed to parse empty.ttl");
        return 0;
    } else if (_mv_360.has_value) {
        __auto_type result = _mv_360.value;
        __auto_type _mv_361 = result;
        switch (_mv_361.tag) {
            case types_ReasonerResult_reason_success:
            {
...
uint8_t test_cli_test_bfo_consistent(slop_arena* arena) {
    {
        ...
        printf("%s\n", "  BFO: parsing...");
        __auto_type _mv_362 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/bfo-core.ttl"));
        if (!_mv_362.is_ok) {
            ...
            printf("%s\n", " triples, indexing...");
            {
                ...
                printf("%s\n", " triples, reasoning...");
                {
                    ...
                    __auto_type _mv_363 = growl_reason(arena, ig);
                    switch (_mv_363.tag) {
                        case types_ReasonerResult_reason_success:
                        {
                            __auto_type s = _mv_363.data.reason_success;
                            {
                        ...
                        case types_ReasonerResult_reason_inconsistent:
                        {
                            __auto_type report = _mv_363.data.reason_inconsistent;
...
uint8_t test_cli_test_invalid_literal_file(slop_arena* arena) {
    __auto_type _mv_364 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/invalid-literal.ttl"));
...
uint8_t test_cli_test_valid_literals_file(slop_arena* arena) {
    __auto_type _mv_366 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/valid-literals.ttl"));
    if (!_mv_366.has_value) {
        ...
        switch (_mv_367.tag) {
            ...
            case types_ReasonerResult_reason_inconsistent:
            {
                __auto_type report = _mv_367.data.reason_inconsistent;
...
uint8_t test_cli_test_emit_roundtrip(slop_arena* arena) {
    __auto_type _mv_368 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/subclass-chain.ttl"));
    if (!_mv_368.has_value) {
        printf("%s\n", "  ERROR: failed to parse for roundtrip");
        return 0;
    } else if (_mv_368.has_value) {
        __auto_type result = _mv_368.value;
        __auto_type _mv_369 = result;
        switch (_mv_369.tag) {
            case types_ReasonerResult_reason_success:
            {
                __auto_type s = _mv_369.data.reason_success;
                {
                    ...
                    __auto_type _mv_370 = serialize_ttl_serialize_ttl_stream(arena, out_graph, config, emit_path);
                    if (!_mv_370.is_ok) {
                        __auto_type _ = _mv_370.data.err;
                        printf("%s\n", "  ERROR: failed to write roundtrip file");
                        return 0;
                    } else if (_mv_370.is_ok) {
                        __auto_type _ = _mv_370.data.ok;
                        __auto_type _mv_371 = ttl_parse_ttl_file(arena, emit_path);
                        if (!_mv_371.is_ok) {
                            __auto_type _ = _mv_371.data.err;
                            printf("%s\n", "  ERROR: failed to re-parse emitted file");
                            return 0;
                        } else if (_mv_371.is_ok) {
                            __auto_type g2 = _mv_371.data.ok;
                            {
...
uint8_t test_cli_test_missing_file(slop_arena* arena) {
    __auto_type _mv_372 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/does-not-exist.ttl"));
    if (!_mv_372.has_value) {
        return 1;
    } else if (_mv_372.has_value) {
        __auto_type _ = _mv_372.value;
...
uint8_t test_cli_test_validate_unsat(slop_arena* arena) {
    __auto_type _mv_373 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/validate-unsat.ttl"));
    if (!_mv_373.is_ok) {
        ...
        __auto_type g = _mv_373.data.ok;
        {
            ...
            switch (_mv_374.tag) {
                case types_ReasonerResult_reason_inconsistent:
                {
                    __auto_type _ = _mv_374.data.reason_inconsistent;
                    return 1;
                }
                case types_ReasonerResult_reason_success:
                {
...
uint8_t test_cli_test_validate_clean(slop_arena* arena) {
    __auto_type _mv_375 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/validate-clean.ttl"));
    if (!_mv_375.is_ok) {
...
uint8_t test_cli_test_no_validate_unsat_passes(slop_arena* arena) {
    __auto_type _mv_377 = test_cli_parse_and_reason(arena, SLOP_STR("fixtures/validate-unsat.ttl"));
...
uint8_t test_cli_test_validate_unsat_prop(slop_arena* arena) {
    __auto_type _mv_379 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/validate-unsat-prop.ttl"));
    if (!_mv_379.is_ok) {
        ...
        __auto_type g = _mv_379.data.ok;
        {
            ...
            switch (_mv_380.tag) {
                ...
                case types_ReasonerResult_reason_success:
                {
                    __auto_type _ = _mv_380.data.reason_success;
...
uint8_t test_cli_test_validate_domain_reports_class(slop_arena* arena) {
    __auto_type _mv_381 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/validate-domain-unsat.ttl"));
    if (!_mv_381.is_ok) {
        ...
        __auto_type g = _mv_381.data.ok;
        {
            ...
            switch (_mv_382.tag) {
                case types_ReasonerResult_reason_inconsistent:
                {
                    __auto_type report = _mv_382.data.reason_inconsistent;
...
uint8_t test_cli_test_validate_ns_scoping(slop_arena* arena) {
    __auto_type _mv_383 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/validate-bg-domain.ttl"));
    if (!_mv_383.is_ok) {
        ...
        __auto_type g = _mv_383.data.ok;
        {
            ...
            switch (_mv_384.tag) {
                case types_ReasonerResult_reason_inconsistent:
                {
                    __auto_type report = _mv_384.data.reason_inconsistent;
                    return strlib_starts_with(report.reason, SLOP_STR("Unsatisfiable class:"));
                }
                case types_ReasonerResult_reason_success:
                {
...
uint8_t test_cli_test_validate_ns_filters_tlo(slop_arena* arena) {
    __auto_type _mv_385 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/validate-bg-tlo.ttl"));
    if (!_mv_385.is_ok) {
        ...
        __auto_type g = _mv_385.data.ok;
        {
            ...
            switch (_mv_386.tag) {
                case types_ReasonerResult_reason_success:
                {
                    __auto_type _ = _mv_386.data.reason_success;
                    return 1;
                }
                case types_ReasonerResult_reason_inconsistent:
                {
                    __auto_type report = _mv_386.data.reason_inconsistent;
                    printf("%s", "  ERROR: unexpected inconsistency: ");
                    printf("%.*s\n", (int)(report.reason).len, (report.reason).data);
                    return 0;
                }
...
uint8_t test_cli_test_background_merge(slop_arena* arena) {
    __auto_type _mv_387 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/validate-bg-tlo.ttl"));
    if (!_mv_387.is_ok) {
        __auto_type _ = _mv_387.data.err;
        printf("%s\n", "  ERROR: failed to parse validate-bg-tlo.ttl");
        return 0;
    } else if (_mv_387.is_ok) {
        __auto_type bg_graph = _mv_387.data.ok;
        __auto_type _mv_388 = ttl_parse_ttl_file(arena, SLOP_STR("fixtures/validate-domain-only.ttl"));
        if (!_mv_388.is_ok) {
            __auto_type _ = _mv_388.data.err;
            printf("%s\n", "  ERROR: failed to parse validate-domain-only.ttl");
            return 0;
        } else if (_mv_388.is_ok) {
            __auto_type domain_graph = _mv_388.data.ok;
            {
                ...
                }
                {
                    ...
                    switch (_mv_389.tag) {
                        case types_ReasonerResult_reason_inconsistent:
                        {
                            __auto_type report = _mv_389.data.reason_inconsistent;
                            return strlib_starts_with(report.reason, SLOP_STR("Unsatisfiable class:"));
                        }
                        case types_ReasonerResult_reason_success:
                        {
                            __auto_type _ = _mv_389.data.reason_success;
