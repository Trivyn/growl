...
static void cls__lambda_96(cls__lambda_96_env_t* _env, rdf_Triple int_triple) { ({ __auto_type int_class = rdf_triple_subject(int_triple); __auto_type list_head = rdf_triple_object(int_triple); __auto_type component_classes = rdf_list_elements_indexed(_env->arena, _env->g, list_head); ({ __auto_type _mv = ({ void* _ptr = slop_map_get(_env->delta.by_predicate, &(_env->type_pred)); _ptr ? (slop_option_ptr){ .has_value = true, .value = *(void**)_ptr } : (slop_option_ptr){ .has_value = false }; }); if (_mv.has_value) { __auto_type pred_triples = _mv.value; ({ slop_map* _coll = (slop_map*)pred_triples; for (size_t _i = 0; _i < _coll->cap; _i++) { if (_coll->entries[_i].occupied) { rdf_Triple dt = *(rdf_Triple*)_coll->entries[_i].key; ({ __auto_type individual = rdf_triple_subject(dt); __auto_type asserted_class = rdf_triple_object(dt); ({ __auto_type is_component = 0; ({ __auto_type _coll = component_classes; for (size_t _i = 0; _i < _coll.len; _i++) { __auto_type comp = _coll.data[_i]; ((rdf_term_eq(comp, asserted_class)) ? ({ ({ is_component = 1; (void)0; }); 0; }) : ({ (void)0; })); } 0; }); ((is_component) ? ({ ({ __auto_type all_satisfied = 1; ({ __auto_type _coll = component_classes; for (size_t _i = 0; _i < _coll.len; _i++) { __auto_type comp = _coll.data[_i]; ({ __auto_type type_triple = rdf_make_triple(_env->arena, individual, _env->type_pred, comp); ((!(rdf_indexed_graph_contains(_env->g, type_triple))) ? ({ ({ all_satisfied = 0; (void)0; }); 0; }) : ({ (void)0; })); }); } 0; }); ((all_satisfied) ? ({ ({ __auto_type inferred = rdf_make_triple(_env->arena, individual, _env->type_pred, int_class); ((!(rdf_indexed_graph_contains(_env->g, inferred))) ? ({ ({ __auto_type _lst_p = &((*_env->result)); __auto_type _item = (inferred); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(_env->arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; }); 0; }) : ({ (void)0; })); }); 0; }) : ({ (void)0; })); }); 0; }) : ({ (void)0; })); }); }); } } 0; }); } else { ({ (void)0; }); } (void)0; }); }); }

typedef struct { slop_arena* arena; index_IndexedGraph g; types_Delta delta; rdf_Term type_pred; slop_list_rdf_Triple* result; } cls__lambda_97_env_t;

static void cls__lambda_97(cls__lambda_97_env_t* _env, rdf_Triple int_triple) { ({ __auto_type int_class = rdf_triple_subject(int_triple); __auto_type list_head = rdf_triple_object(int_triple); __auto_type component_classes = rdf_list_elements_indexed(_env->arena, _env->g, list_head); ({ __auto_type _mv = ({ void* _ptr = slop_map_get(_env->delta.by_predicate, &(_env->type_pred)); _ptr ? (slop_option_ptr){ .has_value = true, .value = *(void**)_ptr } : (slop_option_ptr){ .has_value = false }; }); if (_mv.has_value) { __auto_type pred_triples = _mv.value; ({ slop_map* _coll = (slop_map*)pred_triples; for (size_t _i = 0; _i < _coll->cap; _i++) { if (_coll->entries[_i].occupied) { rdf_Triple dt = *(rdf_Triple*)_coll->entries[_i].key; ((rdf_term_eq(rdf_triple_object(dt), int_class)) ? ({ ({ __auto_type individual = rdf_triple_subject(dt); ({ __auto_type _coll = component_classes; for (size_t _i = 0; _i < _coll.len; _i++) { __auto_type comp = _coll.data[_i]; ({ __auto_type inferred = rdf_make_triple(_env->arena, individual, _env->type_pred, comp); ((!(rdf_indexed_graph_contains(_env->g, inferred))) ? ({ ({ __auto_type _lst_p = &((*_env->result)); __auto_type _item = (inferred); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(_env->arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; }); 0; }) : ({ (void)0; })); }); } 0; }); }); 0; }) : ({ (void)0; })); } } 0; }); } else { ({ (void)0; }); } (void)0; }); }); }

typedef struct { slop_arena* arena; index_IndexedGraph g; types_Delta delta; rdf_Term type_pred; slop_list_rdf_Triple* result; } cls__lambda_98_env_t;

static void cls__lambda_98(cls__lambda_98_env_t* _env, rdf_Triple union_triple) { ({ __auto_type union_class = rdf_triple_subject(union_triple); __auto_type list_head = rdf_triple_object(union_triple); __auto_type component_classes = rdf_list_elements_indexed(_env->arena, _env->g, list_head); ({ __auto_type _mv = ({ void* _ptr = slop_map_get(_env->delta.by_predicate, &(_env->type_pred)); _ptr ? (slop_option_ptr){ .has_value = true, .value = *(void**)_ptr } : (slop_option_ptr){ .has_value = false }; }); if (_mv.has_value) { __auto_type pred_triples = _mv.value; ({ slop_map* _coll = (slop_map*)pred_triples; for (size_t _i = 0; _i < _coll->cap; _i++) { if (_coll->entries[_i].occupied) { rdf_Triple dt = *(rdf_Triple*)_coll->entries[_i].key; ({ __auto_type individual = rdf_triple_subject(dt); __auto_type asserted_class = rdf_triple_object(dt); ({ __auto_type _coll = component_classes; for (size_t _i = 0; _i < _coll.len; _i++) { __auto_type comp = _coll.data[_i]; ((rdf_term_eq(comp, asserted_class)) ? ({ ({ __auto_type inferred = rdf_make_triple(_env->arena, individual, _env->type_pred, union_class); ((!(rdf_indexed_graph_contains(_env->g, inferred))) ? ({ ({ __auto_type _lst_p = &((*_env->result)); __auto_type _item = (inferred); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(_env->arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; }); 0; }) : ({ (void)0; })); }); 0; }) : ({ (void)0; })); } 0; }); }); } } 0; }); } else { ({ (void)0; }); } (void)0; }); }); }

typedef struct { index_IndexedGraph g; rdf_Term on_prop_pred; slop_option_rdf_Term no_term; types_Delta delta; slop_arena* arena; rdf_Term type_pred; slop_list_rdf_Triple* result; } cls__lambda_100_env_t;

typedef struct { types_Delta delta; slop_arena* arena; rdf_Term type_pred; rdf_Term restriction; index_IndexedGraph g; slop_list_rdf_Triple* result; } cls__lambda_101_env_t;

static void cls__lambda_101(cls__lambda_101_env_t* _env, rdf_Triple prop_triple) { ({ __auto_type property = rdf_triple_object(prop_triple); ({ __auto_type _mv = ({ void* _ptr = slop_map_get(_env->delta.by_predicate, &(property)); _ptr ? (slop_option_ptr){ .has_value = true, .value = *(void**)_ptr } : (slop_option_ptr){ .has_value = false }; }); if (_mv.has_value) { __auto_type pred_triples = _mv.value; ({ slop_map* _coll = (slop_map*)pred_triples; for (size_t _i = 0; _i < _coll->cap; _i++) { if (_coll->entries[_i].occupied) { rdf_Triple dt = *(rdf_Triple*)_coll->entries[_i].key; ({ __auto_type u = rdf_triple_subject(dt); __auto_type inferred = rdf_make_triple(_env->arena, u, _env->type_pred, _env->restriction); ((!(rdf_indexed_graph_contains(_env->g, inferred))) ? ({ ({ __auto_type _lst_p = &((*_env->result)); __auto_type _item = (inferred); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(_env->arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; }); 0; }) : ({ (void)0; })); }); } } 0; }); } else { ({ (void)0; }); } (void)0; }); }); }

static void cls__lambda_100(cls__lambda_100_env_t* _env, rdf_Triple svf_triple) { ({ __auto_type restriction = rdf_triple_subject(svf_triple); rdf_indexed_graph_for_each(_env->g, (slop_option_rdf_Term){.has_value = 1, .value = restriction}, (slop_option_rdf_Term){.has_value = 1, .value = _env->on_prop_pred}, _env->no_term, ({ cls__lambda_101_env_t* cls__lambda_101_env = (cls__lambda_101_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_101_env_t)); *cls__lambda_101_env = (cls__lambda_101_env_t){ .delta = _env->delta, .arena = _env->arena, .type_pred = _env->type_pred, .restriction = restriction, .g = _env->g, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_101, (void*)cls__lambda_101_env }; })); }); }

typedef struct { index_IndexedGraph g; rdf_Term on_prop_pred; slop_option_rdf_Term no_term; types_Delta delta; rdf_Term type_pred; slop_arena* arena; slop_list_rdf_Triple* result; } cls__lambda_102_env_t;

typedef struct { types_Delta delta; rdf_Term type_pred; rdf_Term value_class; index_IndexedGraph g; slop_option_rdf_Term no_term; slop_arena* arena; rdf_Term restriction; slop_list_rdf_Triple* result; } cls__lambda_103_env_t;

typedef struct { slop_arena* arena; rdf_Term type_pred; rdf_Term restriction; index_IndexedGraph g; slop_list_rdf_Triple* result; } cls__lambda_104_env_t;

static void cls__lambda_104(cls__lambda_104_env_t* _env, rdf_Triple usage_triple) { ({ __auto_type u = rdf_triple_subject(usage_triple); __auto_type inferred = rdf_make_triple(_env->arena, u, _env->type_pred, _env->restriction); ((!(rdf_indexed_graph_contains(_env->g, inferred))) ? ({ ({ __auto_type _lst_p = &((*_env->result)); __auto_type _item = (inferred); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(_env->arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; }); 0; }) : ({ (void)0; })); }); }

static void cls__lambda_103(cls__lambda_103_env_t* _env, rdf_Triple prop_triple) { ({ __auto_type property = rdf_triple_object(prop_triple); ({ __auto_type _mv = ({ void* _ptr = slop_map_get(_env->delta.by_predicate, &(_env->type_pred)); _ptr ? (slop_option_ptr){ .has_value = true, .value = *(void**)_ptr } : (slop_option_ptr){ .has_value = false }; }); if (_mv.has_value) { __auto_type pred_triples = _mv.value; ({ slop_map* _coll = (slop_map*)pred_triples; for (size_t _i = 0; _i < _coll->cap; _i++) { if (_coll->entries[_i].occupied) { rdf_Triple dt = *(rdf_Triple*)_coll->entries[_i].key; ((rdf_term_eq(rdf_triple_object(dt), _env->value_class)) ? ({ ({ __auto_type v = rdf_triple_subject(dt); rdf_indexed_graph_for_each(_env->g, _env->no_term, (slop_option_rdf_Term){.has_value = 1, .value = property}, (slop_option_rdf_Term){.has_value = 1, .value = v}, ({ cls__lambda_104_env_t* cls__lambda_104_env = (cls__lambda_104_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_104_env_t)); *cls__lambda_104_env = (cls__lambda_104_env_t){ .arena = _env->arena, .type_pred = _env->type_pred, .restriction = _env->restriction, .g = _env->g, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_104, (void*)cls__lambda_104_env }; })); }); 0; }) : ({ (void)0; })); } } 0; }); } else { ({ (void)0; }); } (void)0; }); ({ __auto_type _mv = ({ void* _ptr = slop_map_get(_env->delta.by_predicate, &(property)); _ptr ? (slop_option_ptr){ .has_value = true, .value = *(void**)_ptr } : (slop_option_ptr){ .has_value = false }; }); if (_mv.has_value) { __auto_type pred_triples = _mv.value; ({ slop_map* _coll = (slop_map*)pred_triples; for (size_t _i = 0; _i < _coll->cap; _i++) { if (_coll->entries[_i].occupied) { rdf_Triple dt = *(rdf_Triple*)_coll->entries[_i].key; ({ __auto_type u = rdf_triple_subject(dt); __auto_type v = rdf_triple_object(dt); __auto_type v_type = rdf_make_triple(_env->arena, v, _env->type_pred, _env->value_class); ((rdf_indexed_graph_contains(_env->g, v_type)) ? ({ ({ __auto_type inferred = rdf_make_triple(_env->arena, u, _env->type_pred, _env->restriction); ((!(rdf_indexed_graph_contains(_env->g, inferred))) ? ({ ({ __auto_type _lst_p = &((*_env->result)); __auto_type _item = (inferred); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(_env->arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; }); 0; }) : ({ (void)0; })); }); 0; }) : ({ (void)0; })); }); } } 0; }); } else { ({ (void)0; }); } (void)0; }); }); }

static void cls__lambda_102(cls__lambda_102_env_t* _env, rdf_Triple svf_triple) { ({ __auto_type restriction = rdf_triple_subject(svf_triple); __auto_type value_class = rdf_triple_object(svf_triple); rdf_indexed_graph_for_each(_env->g, (slop_option_rdf_Term){.has_value = 1, .value = restriction}, (slop_option_rdf_Term){.has_value = 1, .value = _env->on_prop_pred}, _env->no_term, ({ cls__lambda_103_env_t* cls__lambda_103_env = (cls__lambda_103_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_103_env_t)); *cls__lambda_103_env = (cls__lambda_103_env_t){ .delta = _env->delta, .type_pred = _env->type_pred, .value_class = value_class, .g = _env->g, .no_term = _env->no_term, .arena = _env->arena, .restriction = restriction, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_103, (void*)cls__lambda_103_env }; })); }); }

typedef struct { index_IndexedGraph g; rdf_Term on_prop_pred; slop_option_rdf_Term no_term; types_Delta delta; rdf_Term type_pred; slop_arena* arena; slop_list_rdf_Triple* result; } cls__lambda_105_env_t;

typedef struct { types_Delta delta; rdf_Term type_pred; rdf_Term restriction; index_IndexedGraph g; slop_option_rdf_Term no_term; slop_arena* arena; rdf_Term value_class; slop_list_rdf_Triple* result; } cls__lambda_106_env_t;

typedef struct { slop_arena* arena; rdf_Term type_pred; rdf_Term value_class; index_IndexedGraph g; slop_list_rdf_Triple* result; } cls__lambda_107_env_t;

static void cls__lambda_107(cls__lambda_107_env_t* _env, rdf_Triple usage_triple) { ({ __auto_type v = rdf_triple_object(usage_triple); __auto_type inferred = rdf_make_triple(_env->arena, v, _env->type_pred, _env->value_class); ((!(rdf_indexed_graph_contains(_env->g, inferred))) ? ({ ({ __auto_type _lst_p = &((*_env->result)); __auto_type _item = (inferred); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(_env->arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; }); 0; }) : ({ (void)0; })); }); }

static void cls__lambda_106(cls__lambda_106_env_t* _env, rdf_Triple prop_triple) { ({ __auto_type property = rdf_triple_object(prop_triple); ({ __auto_type _mv = ({ void* _ptr = slop_map_get(_env->delta.by_predicate, &(_env->type_pred)); _ptr ? (slop_option_ptr){ .has_value = true, .value = *(void**)_ptr } : (slop_option_ptr){ .has_value = false }; }); if (_mv.has_value) { __auto_type pred_triples = _mv.value; ({ slop_map* _coll = (slop_map*)pred_triples; for (size_t _i = 0; _i < _coll->cap; _i++) { if (_coll->entries[_i].occupied) { rdf_Triple dt = *(rdf_Triple*)_coll->entries[_i].key; ((rdf_term_eq(rdf_triple_object(dt), _env->restriction)) ? ({ ({ __auto_type u = rdf_triple_subject(dt); rdf_indexed_graph_for_each(_env->g, (slop_option_rdf_Term){.has_value = 1, .value = u}, (slop_option_rdf_Term){.has_value = 1, .value = property}, _env->no_term, ({ cls__lambda_107_env_t* cls__lambda_107_env = (cls__lambda_107_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_107_env_t)); *cls__lambda_107_env = (cls__lambda_107_env_t){ .arena = _env->arena, .type_pred = _env->type_pred, .value_class = _env->value_class, .g = _env->g, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_107, (void*)cls__lambda_107_env }; })); }); 0; }) : ({ (void)0; })); } } 0; }); } else { ({ (void)0; }); } (void)0; }); ({ __auto_type _mv = ({ void* _ptr = slop_map_get(_env->delta.by_predicate, &(property)); _ptr ? (slop_option_ptr){ .has_value = true, .value = *(void**)_ptr } : (slop_option_ptr){ .has_value = false }; }); if (_mv.has_value) { __auto_type pred_triples = _mv.value; ({ slop_map* _coll = (slop_map*)pred_triples; for (size_t _i = 0; _i < _coll->cap; _i++) { if (_coll->entries[_i].occupied) { rdf_Triple dt = *(rdf_Triple*)_coll->entries[_i].key; ({ __auto_type u = rdf_triple_subject(dt); __auto_type v = rdf_triple_object(dt); __auto_type u_type = rdf_make_triple(_env->arena, u, _env->type_pred, _env->restriction); ((rdf_indexed_graph_contains(_env->g, u_type)) ? ({ ({ __auto_type inferred = rdf_make_triple(_env->arena, v, _env->type_pred, _env->value_class); ((!(rdf_indexed_graph_contains(_env->g, inferred))) ? ({ ({ __auto_type _lst_p = &((*_env->result)); __auto_type _item = (inferred); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(_env->arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; }); 0; }) : ({ (void)0; })); }); 0; }) : ({ (void)0; })); }); } } 0; }); } else { ({ (void)0; }); } (void)0; }); }); }

static void cls__lambda_105(cls__lambda_105_env_t* _env, rdf_Triple avf_triple) { ({ __auto_type restriction = rdf_triple_subject(avf_triple); __auto_type value_class = rdf_triple_object(avf_triple); rdf_indexed_graph_for_each(_env->g, (slop_option_rdf_Term){.has_value = 1, .value = restriction}, (slop_option_rdf_Term){.has_value = 1, .value = _env->on_prop_pred}, _env->no_term, ({ cls__lambda_106_env_t* cls__lambda_106_env = (cls__lambda_106_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_106_env_t)); *cls__lambda_106_env = (cls__lambda_106_env_t){ .delta = _env->delta, .type_pred = _env->type_pred, .restriction = restriction, .g = _env->g, .no_term = _env->no_term, .arena = _env->arena, .value_class = value_class, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_106, (void*)cls__lambda_106_env }; })); }); }

typedef struct { index_IndexedGraph g; rdf_Term on_prop_pred; slop_option_rdf_Term no_term; types_Delta delta; rdf_Term type_pred; slop_arena* arena; slop_list_rdf_Triple* result; } cls__lambda_108_env_t;

typedef struct { types_Delta delta; rdf_Term type_pred; rdf_Term restriction; slop_arena* arena; rdf_Term value; index_IndexedGraph g; slop_list_rdf_Triple* result; } cls__lambda_109_env_t;

static void cls__lambda_109(cls__lambda_109_env_t* _env, rdf_Triple prop_triple) { ({ __auto_type property = rdf_triple_object(prop_triple); ({ __auto_type _mv = ({ void* _ptr = slop_map_get(_env->delta.by_predicate, &(_env->type_pred)); _ptr ? (slop_option_ptr){ .has_value = true, .value = *(void**)_ptr } : (slop_option_ptr){ .has_value = false }; }); if (_mv.has_value) { __auto_type pred_triples = _mv.value; ({ slop_map* _coll = (slop_map*)pred_triples; for (size_t _i = 0; _i < _coll->cap; _i++) { if (_coll->entries[_i].occupied) { rdf_Triple dt = *(rdf_Triple*)_coll->entries[_i].key; ((rdf_term_eq(rdf_triple_object(dt), _env->restriction)) ? ({ ({ __auto_type u = rdf_triple_subject(dt); __auto_type inferred = rdf_make_triple(_env->arena, u, property, _env->value); ((!(rdf_indexed_graph_contains(_env->g, inferred))) ? ({ ({ __auto_type _lst_p = &((*_env->result)); __auto_type _item = (inferred); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(_env->arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; }); 0; }) : ({ (void)0; })); }); 0; }) : ({ (void)0; })); } } 0; }); } else { ({ (void)0; }); } (void)0; }); }); }

static void cls__lambda_108(cls__lambda_108_env_t* _env, rdf_Triple hv_triple) { ({ __auto_type restriction = rdf_triple_subject(hv_triple); __auto_type value = rdf_triple_object(hv_triple); rdf_indexed_graph_for_each(_env->g, (slop_option_rdf_Term){.has_value = 1, .value = restriction}, (slop_option_rdf_Term){.has_value = 1, .value = _env->on_prop_pred}, _env->no_term, ({ cls__lambda_109_env_t* cls__lambda_109_env = (cls__lambda_109_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_109_env_t)); *cls__lambda_109_env = (cls__lambda_109_env_t){ .delta = _env->delta, .type_pred = _env->type_pred, .restriction = restriction, .arena = _env->arena, .value = value, .g = _env->g, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_109, (void*)cls__lambda_109_env }; })); }); }

typedef struct { index_IndexedGraph g; rdf_Term on_prop_pred; slop_option_rdf_Term no_term; types_Delta delta; slop_arena* arena; rdf_Term type_pred; slop_list_rdf_Triple* result; } cls__lambda_110_env_t;

typedef struct { types_Delta delta; rdf_Term value; slop_arena* arena; rdf_Term type_pred; rdf_Term restriction; index_IndexedGraph g; slop_list_rdf_Triple* result; } cls__lambda_111_env_t;

static void cls__lambda_111(cls__lambda_111_env_t* _env, rdf_Triple prop_triple) { ({ __auto_type property = rdf_triple_object(prop_triple); ({ __auto_type _mv = ({ void* _ptr = slop_map_get(_env->delta.by_predicate, &(property)); _ptr ? (slop_option_ptr){ .has_value = true, .value = *(void**)_ptr } : (slop_option_ptr){ .has_value = false }; }); if (_mv.has_value) { __auto_type pred_triples = _mv.value; ({ slop_map* _coll = (slop_map*)pred_triples; for (size_t _i = 0; _i < _coll->cap; _i++) { if (_coll->entries[_i].occupied) { rdf_Triple dt = *(rdf_Triple*)_coll->entries[_i].key; ((rdf_term_eq(rdf_triple_object(dt), _env->value)) ? ({ ({ __auto_type u = rdf_triple_subject(dt); __auto_type inferred = rdf_make_triple(_env->arena, u, _env->type_pred, _env->restriction); ((!(rdf_indexed_graph_contains(_env->g, inferred))) ? ({ ({ __auto_type _lst_p = &((*_env->result)); __auto_type _item = (inferred); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(_env->arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; }); 0; }) : ({ (void)0; })); }); 0; }) : ({ (void)0; })); } } 0; }); } else { ({ (void)0; }); } (void)0; }); }); }

static void cls__lambda_110(cls__lambda_110_env_t* _env, rdf_Triple hv_triple) { ({ __auto_type restriction = rdf_triple_subject(hv_triple); __auto_type value = rdf_triple_object(hv_triple); rdf_indexed_graph_for_each(_env->g, (slop_option_rdf_Term){.has_value = 1, .value = restriction}, (slop_option_rdf_Term){.has_value = 1, .value = _env->on_prop_pred}, _env->no_term, ({ cls__lambda_111_env_t* cls__lambda_111_env = (cls__lambda_111_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_111_env_t)); *cls__lambda_111_env = (cls__lambda_111_env_t){ .delta = _env->delta, .value = value, .arena = _env->arena, .type_pred = _env->type_pred, .restriction = restriction, .g = _env->g, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_111, (void*)cls__lambda_111_env }; })); }); }

typedef struct { index_IndexedGraph g; rdf_Term on_prop_pred; slop_option_rdf_Term no_term; rdf_Term type_pred; slop_arena* arena; rdf_Term same_as; slop_list_rdf_Triple* result; } cls__lambda_112_env_t;

typedef struct { index_IndexedGraph g; slop_option_rdf_Term no_term; rdf_Term type_pred; rdf_Term restriction; slop_arena* arena; rdf_Term same_as; slop_list_rdf_Triple* result; } cls__lambda_113_env_t;

typedef struct { index_IndexedGraph g; rdf_Term property; slop_option_rdf_Term no_term; slop_arena* arena; rdf_Term same_as; slop_list_rdf_Triple* result; } cls__lambda_114_env_t;

typedef struct { slop_option_rdf_Term first_val; slop_arena* arena; rdf_Term same_as; index_IndexedGraph g; slop_list_rdf_Triple* result; } cls__lambda_115_env_t;

static void cls__lambda_115(cls__lambda_115_env_t* _env, rdf_Triple value_triple) { ({ __auto_type val = rdf_triple_object(value_triple); ({ __auto_type _mv = _env->first_val; if (_mv.has_value) { __auto_type fv = _mv.value; ({ __auto_type inferred = rdf_make_triple(_env->arena, fv, _env->same_as, val); ((!(rdf_indexed_graph_contains(_env->g, inferred))) ? ({ ({ __auto_type _lst_p = &((*_env->result)); __auto_type _item = (inferred); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(_env->arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; }); 0; }) : ({ (void)0; })); }); } else { ({ _env->first_val = (slop_option_rdf_Term){.has_value = 1, .value = val}; (void)0; }); } (void)0; }); }); }

static void cls__lambda_114(cls__lambda_114_env_t* _env, rdf_Triple type_triple) { ({ __auto_type individual = rdf_triple_subject(type_triple); slop_option_rdf_Term first_val = (slop_option_rdf_Term){.has_value = false}; rdf_indexed_graph_for_each(_env->g, (slop_option_rdf_Term){.has_value = 1, .value = individual}, (slop_option_rdf_Term){.has_value = 1, .value = _env->property}, _env->no_term, ({ cls__lambda_115_env_t* cls__lambda_115_env = (cls__lambda_115_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_115_env_t)); *cls__lambda_115_env = (cls__lambda_115_env_t){ .first_val = first_val, .arena = _env->arena, .same_as = _env->same_as, .g = _env->g, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_115, (void*)cls__lambda_115_env }; })); }); }

static void cls__lambda_113(cls__lambda_113_env_t* _env, rdf_Triple prop_triple) { ({ __auto_type property = rdf_triple_object(prop_triple); rdf_indexed_graph_for_each(_env->g, _env->no_term, (slop_option_rdf_Term){.has_value = 1, .value = _env->type_pred}, (slop_option_rdf_Term){.has_value = 1, .value = _env->restriction}, ({ cls__lambda_114_env_t* cls__lambda_114_env = (cls__lambda_114_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_114_env_t)); *cls__lambda_114_env = (cls__lambda_114_env_t){ .g = _env->g, .property = property, .no_term = _env->no_term, .arena = _env->arena, .same_as = _env->same_as, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_114, (void*)cls__lambda_114_env }; })); }); }

static void cls__lambda_112(cls__lambda_112_env_t* _env, rdf_Triple maxc_triple) { ({ __auto_type restriction = rdf_triple_subject(maxc_triple); rdf_indexed_graph_for_each(_env->g, (slop_option_rdf_Term){.has_value = 1, .value = restriction}, (slop_option_rdf_Term){.has_value = 1, .value = _env->on_prop_pred}, _env->no_term, ({ cls__lambda_113_env_t* cls__lambda_113_env = (cls__lambda_113_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_113_env_t)); *cls__lambda_113_env = (cls__lambda_113_env_t){ .g = _env->g, .no_term = _env->no_term, .type_pred = _env->type_pred, .restriction = restriction, .arena = _env->arena, .same_as = _env->same_as, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_113, (void*)cls__lambda_113_env }; })); }); }

typedef struct { index_IndexedGraph g; rdf_Term on_prop_pred; slop_option_rdf_Term no_term; rdf_Term on_class_pred; rdf_Term thing; rdf_Term type_pred; slop_arena* arena; rdf_Term same_as; slop_list_rdf_Triple* result; } cls__lambda_116_env_t;

typedef struct { index_IndexedGraph g; rdf_Term restriction; rdf_Term on_class_pred; slop_option_rdf_Term no_term; rdf_Term thing; rdf_Term type_pred; slop_arena* arena; rdf_Term same_as; slop_list_rdf_Triple* result; } cls__lambda_117_env_t;

typedef struct { rdf_Term thing; index_IndexedGraph g; slop_option_rdf_Term no_term; rdf_Term type_pred; rdf_Term restriction; rdf_Term property; slop_arena* arena; rdf_Term same_as; slop_list_rdf_Triple* result; } cls__lambda_118_env_t;

typedef struct { index_IndexedGraph g; rdf_Term property; slop_option_rdf_Term no_term; slop_arena* arena; rdf_Term type_pred; rdf_Term qual_class; rdf_Term same_as; slop_list_rdf_Triple* result; } cls__lambda_119_env_t;

typedef struct { slop_arena* arena; rdf_Term type_pred; rdf_Term qual_class; index_IndexedGraph g; slop_option_rdf_Term first_qualified; rdf_Term same_as; slop_list_rdf_Triple* result; } cls__lambda_120_env_t;

static void cls__lambda_120(cls__lambda_120_env_t* _env, rdf_Triple val_triple) { ({ __auto_type y = rdf_triple_object(val_triple); __auto_type y_type = rdf_make_triple(_env->arena, y, _env->type_pred, _env->qual_class); ((rdf_indexed_graph_contains(_env->g, y_type)) ? ({ ({ __auto_type _mv = _env->first_qualified; if (_mv.has_value) { __auto_type fv = _mv.value; ({ __auto_type inferred = rdf_make_triple(_env->arena, fv, _env->same_as, y); ((!(rdf_indexed_graph_contains(_env->g, inferred))) ? ({ ({ __auto_type _lst_p = &((*_env->result)); __auto_type _item = (inferred); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(_env->arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; }); 0; }) : ({ (void)0; })); }); } else { ({ _env->first_qualified = (slop_option_rdf_Term){.has_value = 1, .value = y}; (void)0; }); } (void)0; }); 0; }) : ({ (void)0; })); }); }

static void cls__lambda_119(cls__lambda_119_env_t* _env, rdf_Triple inst_triple) { ({ __auto_type u = rdf_triple_subject(inst_triple); slop_option_rdf_Term first_qualified = (slop_option_rdf_Term){.has_value = false}; rdf_indexed_graph_for_each(_env->g, (slop_option_rdf_Term){.has_value = 1, .value = u}, (slop_option_rdf_Term){.has_value = 1, .value = _env->property}, _env->no_term, ({ cls__lambda_120_env_t* cls__lambda_120_env = (cls__lambda_120_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_120_env_t)); *cls__lambda_120_env = (cls__lambda_120_env_t){ .arena = _env->arena, .type_pred = _env->type_pred, .qual_class = _env->qual_class, .g = _env->g, .first_qualified = first_qualified, .same_as = _env->same_as, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_120, (void*)cls__lambda_120_env }; })); }); }

static void cls__lambda_118(cls__lambda_118_env_t* _env, rdf_Triple class_triple) { ({ __auto_type qual_class = rdf_triple_object(class_triple); ((!(rdf_term_eq(qual_class, _env->thing))) ? ({ rdf_indexed_graph_for_each(_env->g, _env->no_term, (slop_option_rdf_Term){.has_value = 1, .value = _env->type_pred}, (slop_option_rdf_Term){.has_value = 1, .value = _env->restriction}, ({ cls__lambda_119_env_t* cls__lambda_119_env = (cls__lambda_119_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_119_env_t)); *cls__lambda_119_env = (cls__lambda_119_env_t){ .g = _env->g, .property = _env->property, .no_term = _env->no_term, .arena = _env->arena, .type_pred = _env->type_pred, .qual_class = qual_class, .same_as = _env->same_as, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_119, (void*)cls__lambda_119_env }; })); 0; }) : ({ (void)0; })); }); }

static void cls__lambda_117(cls__lambda_117_env_t* _env, rdf_Triple prop_triple) { ({ __auto_type property = rdf_triple_object(prop_triple); rdf_indexed_graph_for_each(_env->g, (slop_option_rdf_Term){.has_value = 1, .value = _env->restriction}, (slop_option_rdf_Term){.has_value = 1, .value = _env->on_class_pred}, _env->no_term, ({ cls__lambda_118_env_t* cls__lambda_118_env = (cls__lambda_118_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_118_env_t)); *cls__lambda_118_env = (cls__lambda_118_env_t){ .thing = _env->thing, .g = _env->g, .no_term = _env->no_term, .type_pred = _env->type_pred, .restriction = _env->restriction, .property = property, .arena = _env->arena, .same_as = _env->same_as, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_118, (void*)cls__lambda_118_env }; })); }); }

static void cls__lambda_116(cls__lambda_116_env_t* _env, rdf_Triple mqc_triple) { ({ __auto_type restriction = rdf_triple_subject(mqc_triple); rdf_indexed_graph_for_each(_env->g, (slop_option_rdf_Term){.has_value = 1, .value = restriction}, (slop_option_rdf_Term){.has_value = 1, .value = _env->on_prop_pred}, _env->no_term, ({ cls__lambda_117_env_t* cls__lambda_117_env = (cls__lambda_117_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_117_env_t)); *cls__lambda_117_env = (cls__lambda_117_env_t){ .g = _env->g, .restriction = restriction, .on_class_pred = _env->on_class_pred, .no_term = _env->no_term, .thing = _env->thing, .type_pred = _env->type_pred, .arena = _env->arena, .same_as = _env->same_as, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_117, (void*)cls__lambda_117_env }; })); }); }

typedef struct { index_IndexedGraph g; rdf_Term on_prop_pred; slop_option_rdf_Term no_term; rdf_Term on_class_pred; rdf_Term thing; rdf_Term type_pred; slop_arena* arena; rdf_Term same_as; slop_list_rdf_Triple* result; } cls__lambda_121_env_t;

typedef struct { index_IndexedGraph g; rdf_Term restriction; rdf_Term on_class_pred; slop_option_rdf_Term no_term; rdf_Term thing; rdf_Term type_pred; slop_arena* arena; rdf_Term same_as; slop_list_rdf_Triple* result; } cls__lambda_122_env_t;

typedef struct { rdf_Term thing; index_IndexedGraph g; slop_option_rdf_Term no_term; rdf_Term type_pred; rdf_Term restriction; rdf_Term property; slop_arena* arena; rdf_Term same_as; slop_list_rdf_Triple* result; } cls__lambda_123_env_t;

typedef struct { index_IndexedGraph g; rdf_Term property; slop_option_rdf_Term no_term; slop_arena* arena; rdf_Term same_as; slop_list_rdf_Triple* result; } cls__lambda_124_env_t;

typedef struct { slop_option_rdf_Term first_val; slop_arena* arena; rdf_Term same_as; index_IndexedGraph g; slop_list_rdf_Triple* result; } cls__lambda_125_env_t;

static void cls__lambda_125(cls__lambda_125_env_t* _env, rdf_Triple val_triple) { ({ __auto_type y = rdf_triple_object(val_triple); ({ __auto_type _mv = _env->first_val; if (_mv.has_value) { __auto_type fv = _mv.value; ({ __auto_type inferred = rdf_make_triple(_env->arena, fv, _env->same_as, y); ((!(rdf_indexed_graph_contains(_env->g, inferred))) ? ({ ({ __auto_type _lst_p = &((*_env->result)); __auto_type _item = (inferred); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(_env->arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; }); 0; }) : ({ (void)0; })); }); } else { ({ _env->first_val = (slop_option_rdf_Term){.has_value = 1, .value = y}; (void)0; }); } (void)0; }); }); }

static void cls__lambda_124(cls__lambda_124_env_t* _env, rdf_Triple inst_triple) { ({ __auto_type u = rdf_triple_subject(inst_triple); slop_option_rdf_Term first_val = (slop_option_rdf_Term){.has_value = false}; rdf_indexed_graph_for_each(_env->g, (slop_option_rdf_Term){.has_value = 1, .value = u}, (slop_option_rdf_Term){.has_value = 1, .value = _env->property}, _env->no_term, ({ cls__lambda_125_env_t* cls__lambda_125_env = (cls__lambda_125_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_125_env_t)); *cls__lambda_125_env = (cls__lambda_125_env_t){ .first_val = first_val, .arena = _env->arena, .same_as = _env->same_as, .g = _env->g, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_125, (void*)cls__lambda_125_env }; })); }); }

static void cls__lambda_123(cls__lambda_123_env_t* _env, rdf_Triple class_triple) { ({ __auto_type qual_class = rdf_triple_object(class_triple); ((rdf_term_eq(qual_class, _env->thing)) ? ({ rdf_indexed_graph_for_each(_env->g, _env->no_term, (slop_option_rdf_Term){.has_value = 1, .value = _env->type_pred}, (slop_option_rdf_Term){.has_value = 1, .value = _env->restriction}, ({ cls__lambda_124_env_t* cls__lambda_124_env = (cls__lambda_124_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_124_env_t)); *cls__lambda_124_env = (cls__lambda_124_env_t){ .g = _env->g, .property = _env->property, .no_term = _env->no_term, .arena = _env->arena, .same_as = _env->same_as, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_124, (void*)cls__lambda_124_env }; })); 0; }) : ({ (void)0; })); }); }

static void cls__lambda_122(cls__lambda_122_env_t* _env, rdf_Triple prop_triple) { ({ __auto_type property = rdf_triple_object(prop_triple); rdf_indexed_graph_for_each(_env->g, (slop_option_rdf_Term){.has_value = 1, .value = _env->restriction}, (slop_option_rdf_Term){.has_value = 1, .value = _env->on_class_pred}, _env->no_term, ({ cls__lambda_123_env_t* cls__lambda_123_env = (cls__lambda_123_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_123_env_t)); *cls__lambda_123_env = (cls__lambda_123_env_t){ .thing = _env->thing, .g = _env->g, .no_term = _env->no_term, .type_pred = _env->type_pred, .restriction = _env->restriction, .property = property, .arena = _env->arena, .same_as = _env->same_as, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_123, (void*)cls__lambda_123_env }; })); }); }

static void cls__lambda_121(cls__lambda_121_env_t* _env, rdf_Triple mqc_triple) { ({ __auto_type restriction = rdf_triple_subject(mqc_triple); rdf_indexed_graph_for_each(_env->g, (slop_option_rdf_Term){.has_value = 1, .value = restriction}, (slop_option_rdf_Term){.has_value = 1, .value = _env->on_prop_pred}, _env->no_term, ({ cls__lambda_122_env_t* cls__lambda_122_env = (cls__lambda_122_env_t*)slop_arena_alloc(_env->arena, sizeof(cls__lambda_122_env_t)); *cls__lambda_122_env = (cls__lambda_122_env_t){ .g = _env->g, .restriction = restriction, .on_class_pred = _env->on_class_pred, .no_term = _env->no_term, .thing = _env->thing, .type_pred = _env->type_pred, .arena = _env->arena, .same_as = _env->same_as, .result = &((*_env->result)) }; (slop_closure_t){ (void*)cls__lambda_122, (void*)cls__lambda_122_env }; })); }); }
...
slop_result_types_Delta_types_InconsistencyReport cls_apply_cls_rules(slop_arena* arena, index_IndexedGraph g, types_Delta delta, uint8_t fast) {
    ...
    slop_result_types_Delta_types_InconsistencyReport _retval;
    {
        ...
        __auto_type _mv_91 = cls_cls_nothing2(arena, g, delta);
        if (_mv_91.has_value) {
        ...
        __auto_type _mv_92 = cls_cls_com(arena, g, delta);
        if (_mv_92.has_value) {
        ...
        if (!(fast)) {
            ...
            __auto_type _mv_93 = cls_cls_maxqc1(arena, g, delta);
            if (_mv_93.has_value) {
                __auto_type report = _mv_93.value;
                return ((slop_result_types_Delta_types_InconsistencyReport){ .is_ok = false, .data.err = report });
            } else if (!_mv_93.has_value) {
            }
            __auto_type _mv_94 = cls_cls_maxqc2(arena, g, delta);
            if (_mv_94.has_value) {
...
slop_option_types_InconsistencyReport cls_cls_nothing2(slop_arena* arena, index_IndexedGraph g, types_Delta delta) {
    ...
    slop_option_types_InconsistencyReport _retval;
    {
        __auto_type type_pred = rdf_make_iri(arena, vocab_RDF_TYPE);
        __auto_type nothing = rdf_make_iri(arena, vocab_OWL_NOTHING);
        __auto_type _mv_95 = ({ void* _ptr = slop_map_get(delta.by_predicate, &(type_pred)); _ptr ? (slop_option_ptr){ .has_value = true, .value = *(void**)_ptr } : (slop_option_ptr){ .has_value = false }; });
        if (_mv_95.has_value) {
            __auto_type type_triples = _mv_95.value;
            {
...
slop_list_rdf_Triple cls_cls_int2(slop_arena* arena, index_IndexedGraph g, types_Delta delta) {
    ...
    slop_list_rdf_Triple _retval;
    {
        ...
        slop_option_rdf_Term no_term = (slop_option_rdf_Term){.has_value = false};
        __auto_type result = ((slop_list_rdf_Triple){ .data = (rdf_Triple*)slop_arena_alloc(arena, 16 * sizeof(rdf_Triple)), .len = 0, .cap = 16 });
        rdf_indexed_graph_for_each(g, no_term, (slop_option_rdf_Term){.has_value = 1, .value = intersection_pred}, no_term, ({ cls__lambda_97_env_t* cls__lambda_97_env = (cls__lambda_97_env_t*)slop_arena_alloc(arena, sizeof(cls__lambda_97_env_t)); *cls__lambda_97_env = (cls__lambda_97_env_t){ .arena = arena, .g = g, .delta = delta, .type_pred = type_pred, .result = &(result) }; (slop_closure_t){ (void*)cls__lambda_97, (void*)cls__lambda_97_env }; }));
        _retval = result;
    }
    SLOP_POST(((((int64_t)((_retval).len)) >= 0)), "(>= (list-len $result) 0)");
    SLOP_POST((types_all_triples_have_predicate(_retval, vocab_RDF_TYPE)), "(all-triples-have-predicate $result RDF_TYPE)");
...
slop_list_rdf_Triple cls_cls_uni(slop_arena* arena, index_IndexedGraph g, types_Delta delta) {
    ...
    slop_list_rdf_Triple _retval;
    {
        ...
        slop_option_rdf_Term no_term = (slop_option_rdf_Term){.has_value = false};
        __auto_type result = ((slop_list_rdf_Triple){ .data = (rdf_Triple*)slop_arena_alloc(arena, 16 * sizeof(rdf_Triple)), .len = 0, .cap = 16 });
        rdf_indexed_graph_for_each(g, no_term, (slop_option_rdf_Term){.has_value = 1, .value = union_pred}, no_term, ({ cls__lambda_98_env_t* cls__lambda_98_env = (cls__lambda_98_env_t*)slop_arena_alloc(arena, sizeof(cls__lambda_98_env_t)); *cls__lambda_98_env = (cls__lambda_98_env_t){ .arena = arena, .g = g, .delta = delta, .type_pred = type_pred, .result = &(result) }; (slop_closure_t){ (void*)cls__lambda_98, (void*)cls__lambda_98_env }; }));
        _retval = result;
    }
    SLOP_POST(((((int64_t)((_retval).len)) >= 0)), "(>= (list-len $result) 0)");
    SLOP_POST((types_all_triples_have_predicate(_retval, vocab_RDF_TYPE)), "(all-triples-have-predicate $result RDF_TYPE)");
            ...
            __auto_type comp_triples = rdf_indexed_graph_match(arena, g, no_term, (slop_option_rdf_Term){.has_value = 1, .value = complement_pred}, no_term);
            {
                ...
                for (size_t _i = 0; _i < _coll.len; _i++) {
                    __auto_type comp_triple = _coll.data[_i];
                    {
                        ...
                        __auto_type _mv_99 = ({ void* _ptr = slop_map_get(delta.by_predicate, &(type_pred)); _ptr ? (slop_option_ptr){ .has_value = true, .value = *(void**)_ptr } : (slop_option_ptr){ .has_value = false }; });
                        if (_mv_99.has_value) {
                            __auto_type type_triples = _mv_99.value;
                            {
                                slop_map* _coll = (slop_map*)type_triples;
                                for (size_t _i = 0; _i < _coll->cap; _i++) {
                                    if (_coll->entries[_i].occupied) {
...
slop_list_rdf_Triple cls_cls_svf1(slop_arena* arena, index_IndexedGraph g, types_Delta delta) {
    ...
    slop_list_rdf_Triple _retval;
    {
        ...
        slop_option_rdf_Term no_term = (slop_option_rdf_Term){.has_value = false};
        __auto_type result = ((slop_list_rdf_Triple){ .data = (rdf_Triple*)slop_arena_alloc(arena, 16 * sizeof(rdf_Triple)), .len = 0, .cap = 16 });
        rdf_indexed_graph_for_each(g, no_term, (slop_option_rdf_Term){.has_value = 1, .value = svf_pred}, (slop_option_rdf_Term){.has_value = 1, .value = thing}, ({ cls__lambda_100_env_t* cls__lambda_100_env = (cls__lambda_100_env_t*)slop_arena_alloc(arena, sizeof(cls__lambda_100_env_t)); *cls__lambda_100_env = (cls__lambda_100_env_t){ .g = g, .on_prop_pred = on_prop_pred, .no_term = no_term, .delta = delta, .arena = arena, .type_pred = type_pred, .result = &(result) }; (slop_closure_t){ (void*)cls__lambda_100, (void*)cls__lambda_100_env }; }));
        _retval = result;
    }
    SLOP_POST(((((int64_t)((_retval).len)) >= 0)), "(>= (list-len $result) 0)");
    SLOP_POST((types_all_triples_have_predicate(_retval, vocab_RDF_TYPE)), "(all-triples-have-predicate $result RDF_TYPE)");
...
slop_list_rdf_Triple cls_cls_svf2(slop_arena* arena, index_IndexedGraph g, types_Delta delta) {
    ...
    slop_list_rdf_Triple _retval;
    {
        ...
        slop_option_rdf_Term no_term = (slop_option_rdf_Term){.has_value = false};
        __auto_type result = ((slop_list_rdf_Triple){ .data = (rdf_Triple*)slop_arena_alloc(arena, 16 * sizeof(rdf_Triple)), .len = 0, .cap = 16 });
        rdf_indexed_graph_for_each(g, no_term, (slop_option_rdf_Term){.has_value = 1, .value = svf_pred}, no_term, ({ cls__lambda_102_env_t* cls__lambda_102_env = (cls__lambda_102_env_t*)slop_arena_alloc(arena, sizeof(cls__lambda_102_env_t)); *cls__lambda_102_env = (cls__lambda_102_env_t){ .g = g, .on_prop_pred = on_prop_pred, .no_term = no_term, .delta = delta, .type_pred = type_pred, .arena = arena, .result = &(result) }; (slop_closure_t){ (void*)cls__lambda_102, (void*)cls__lambda_102_env }; }));
        _retval = result;
    }
    SLOP_POST(((((int64_t)((_retval).len)) >= 0)), "(>= (list-len $result) 0)");
    SLOP_POST((types_all_triples_have_predicate(_retval, vocab_RDF_TYPE)), "(all-triples-have-predicate $result RDF_TYPE)");
...
slop_list_rdf_Triple cls_cls_avf(slop_arena* arena, index_IndexedGraph g, types_Delta delta) {
    ...
    slop_list_rdf_Triple _retval;
    {
        ...
        slop_option_rdf_Term no_term = (slop_option_rdf_Term){.has_value = false};
        __auto_type result = ((slop_list_rdf_Triple){ .data = (rdf_Triple*)slop_arena_alloc(arena, 16 * sizeof(rdf_Triple)), .len = 0, .cap = 16 });
        rdf_indexed_graph_for_each(g, no_term, (slop_option_rdf_Term){.has_value = 1, .value = avf_pred}, no_term, ({ cls__lambda_105_env_t* cls__lambda_105_env = (cls__lambda_105_env_t*)slop_arena_alloc(arena, sizeof(cls__lambda_105_env_t)); *cls__lambda_105_env = (cls__lambda_105_env_t){ .g = g, .on_prop_pred = on_prop_pred, .no_term = no_term, .delta = delta, .type_pred = type_pred, .arena = arena, .result = &(result) }; (slop_closure_t){ (void*)cls__lambda_105, (void*)cls__lambda_105_env }; }));
        _retval = result;
    }
    SLOP_POST(((((int64_t)((_retval).len)) >= 0)), "(>= (list-len $result) 0)");
    SLOP_POST((types_all_triples_have_predicate(_retval, vocab_RDF_TYPE)), "(all-triples-have-predicate $result RDF_TYPE)");
...
slop_list_rdf_Triple cls_cls_hv1(slop_arena* arena, index_IndexedGraph g, types_Delta delta) {
    ...
    slop_list_rdf_Triple _retval;
    {
        ...
        slop_option_rdf_Term no_term = (slop_option_rdf_Term){.has_value = false};
        __auto_type result = ((slop_list_rdf_Triple){ .data = (rdf_Triple*)slop_arena_alloc(arena, 16 * sizeof(rdf_Triple)), .len = 0, .cap = 16 });
        rdf_indexed_graph_for_each(g, no_term, (slop_option_rdf_Term){.has_value = 1, .value = hv_pred}, no_term, ({ cls__lambda_108_env_t* cls__lambda_108_env = (cls__lambda_108_env_t*)slop_arena_alloc(arena, sizeof(cls__lambda_108_env_t)); *cls__lambda_108_env = (cls__lambda_108_env_t){ .g = g, .on_prop_pred = on_prop_pred, .no_term = no_term, .delta = delta, .type_pred = type_pred, .arena = arena, .result = &(result) }; (slop_closure_t){ (void*)cls__lambda_108, (void*)cls__lambda_108_env }; }));
        _retval = result;
    }
    SLOP_POST(((((int64_t)((_retval).len)) >= 0)), "(>= (list-len $result) 0)");
    return _retval;
...
slop_list_rdf_Triple cls_cls_hv2(slop_arena* arena, index_IndexedGraph g, types_Delta delta) {
    ...
    slop_list_rdf_Triple _retval;
    {
        ...
        slop_option_rdf_Term no_term = (slop_option_rdf_Term){.has_value = false};
        __auto_type result = ((slop_list_rdf_Triple){ .data = (rdf_Triple*)slop_arena_alloc(arena, 16 * sizeof(rdf_Triple)), .len = 0, .cap = 16 });
        rdf_indexed_graph_for_each(g, no_term, (slop_option_rdf_Term){.has_value = 1, .value = hv_pred}, no_term, ({ cls__lambda_110_env_t* cls__lambda_110_env = (cls__lambda_110_env_t*)slop_arena_alloc(arena, sizeof(cls__lambda_110_env_t)); *cls__lambda_110_env = (cls__lambda_110_env_t){ .g = g, .on_prop_pred = on_prop_pred, .no_term = no_term, .delta = delta, .arena = arena, .type_pred = type_pred, .result = &(result) }; (slop_closure_t){ (void*)cls__lambda_110, (void*)cls__lambda_110_env }; }));
        _retval = result;
    }
    SLOP_POST(((((int64_t)((_retval).len)) >= 0)), "(>= (list-len $result) 0)");
    SLOP_POST((types_all_triples_have_predicate(_retval, vocab_RDF_TYPE)), "(all-triples-have-predicate $result RDF_TYPE)");
...
slop_list_rdf_Triple cls_cls_maxc2(slop_arena* arena, index_IndexedGraph g, types_Delta delta) {
    ...
    slop_list_rdf_Triple _retval;
    {
        ...
        slop_option_rdf_Term no_term = (slop_option_rdf_Term){.has_value = false};
        __auto_type result = ((slop_list_rdf_Triple){ .data = (rdf_Triple*)slop_arena_alloc(arena, 16 * sizeof(rdf_Triple)), .len = 0, .cap = 16 });
        rdf_indexed_graph_for_each(g, no_term, (slop_option_rdf_Term){.has_value = 1, .value = maxc_pred}, no_term, ({ cls__lambda_112_env_t* cls__lambda_112_env = (cls__lambda_112_env_t*)slop_arena_alloc(arena, sizeof(cls__lambda_112_env_t)); *cls__lambda_112_env = (cls__lambda_112_env_t){ .g = g, .on_prop_pred = on_prop_pred, .no_term = no_term, .type_pred = type_pred, .arena = arena, .same_as = same_as, .result = &(result) }; (slop_closure_t){ (void*)cls__lambda_112, (void*)cls__lambda_112_env }; }));
        _retval = result;
    }
    SLOP_POST(((((int64_t)((_retval).len)) >= 0)), "(>= (list-len $result) 0)");
...
slop_list_rdf_Triple cls_cls_maxqc3(slop_arena* arena, index_IndexedGraph g, types_Delta delta) {
    ...
    slop_list_rdf_Triple _retval;
    {
        ...
        slop_option_rdf_Term no_term = (slop_option_rdf_Term){.has_value = false};
        __auto_type result = ((slop_list_rdf_Triple){ .data = (rdf_Triple*)slop_arena_alloc(arena, 16 * sizeof(rdf_Triple)), .len = 0, .cap = 16 });
        rdf_indexed_graph_for_each(g, no_term, (slop_option_rdf_Term){.has_value = 1, .value = mqc_pred}, no_term, ({ cls__lambda_116_env_t* cls__lambda_116_env = (cls__lambda_116_env_t*)slop_arena_alloc(arena, sizeof(cls__lambda_116_env_t)); *cls__lambda_116_env = (cls__lambda_116_env_t){ .g = g, .on_prop_pred = on_prop_pred, .no_term = no_term, .on_class_pred = on_class_pred, .thing = thing, .type_pred = type_pred, .arena = arena, .same_as = same_as, .result = &(result) }; (slop_closure_t){ (void*)cls__lambda_116, (void*)cls__lambda_116_env }; }));
        _retval = result;
    }
    SLOP_POST(((((int64_t)((_retval).len)) >= 0)), "(>= (list-len $result) 0)");
    SLOP_POST((types_all_triples_have_predicate(_retval, vocab_OWL_SAME_AS)), "(all-triples-have-predicate $result OWL_SAME_AS)");
...
slop_list_rdf_Triple cls_cls_maxqc4(slop_arena* arena, index_IndexedGraph g, types_Delta delta) {
    ...
    slop_list_rdf_Triple _retval;
    {
        ...
        slop_option_rdf_Term no_term = (slop_option_rdf_Term){.has_value = false};
        __auto_type result = ((slop_list_rdf_Triple){ .data = (rdf_Triple*)slop_arena_alloc(arena, 16 * sizeof(rdf_Triple)), .len = 0, .cap = 16 });
        rdf_indexed_graph_for_each(g, no_term, (slop_option_rdf_Term){.has_value = 1, .value = mqc_pred}, no_term, ({ cls__lambda_121_env_t* cls__lambda_121_env = (cls__lambda_121_env_t*)slop_arena_alloc(arena, sizeof(cls__lambda_121_env_t)); *cls__lambda_121_env = (cls__lambda_121_env_t){ .g = g, .on_prop_pred = on_prop_pred, .no_term = no_term, .on_class_pred = on_class_pred, .thing = thing, .type_pred = type_pred, .arena = arena, .same_as = same_as, .result = &(result) }; (slop_closure_t){ (void*)cls__lambda_121, (void*)cls__lambda_121_env }; }));
        _retval = result;
    }
    SLOP_POST(((((int64_t)((_retval).len)) >= 0)), "(>= (list-len $result) 0)");
    SLOP_POST((types_all_triples_have_predicate(_retval, vocab_OWL_SAME_AS)), "(all-triples-have-predicate $result OWL_SAME_AS)");
...
slop_list_rdf_Triple cls_cls_oo(slop_arena* arena, index_IndexedGraph g, types_Delta delta) {
    ...
    slop_list_rdf_Triple _retval;
    {
        ...
        __auto_type type_pred = rdf_make_iri(arena, vocab_RDF_TYPE);
        __auto_type result = ((slop_list_rdf_Triple){ .data = (rdf_Triple*)slop_arena_alloc(arena, 16 * sizeof(rdf_Triple)), .len = 0, .cap = 16 });
        __auto_type _mv_126 = ({ void* _ptr = slop_map_get(delta.by_predicate, &(one_of_pred)); _ptr ? (slop_option_ptr){ .has_value = true, .value = *(void**)_ptr } : (slop_option_ptr){ .has_value = false }; });
        if (_mv_126.has_value) {
            __auto_type pred_triples = _mv_126.value;
            {
