    ...
    slop_string _retval;
    {
        ...
        slop_option_string compressed = (slop_option_string){.has_value = false};
        {
            ...
            for (size_t _i = 0; _i < _coll.len; _i++) {
                ...
                __auto_type _mv_216 = compressed;
                if (_mv_216.has_value) {
                    __auto_type _ = _mv_216.value;
        ...
        __auto_type _mv_217 = compressed;
...
slop_string serialize_ttl_serialize_literal(slop_arena* arena, rdf_Literal lit) {
    slop_string _retval;
    {
        ...
        if (_mv_218.has_value) {
            ...
            __auto_type _mv_219 = dt;
            if (_mv_219.has_value) {
                __auto_type dt_iri = _mv_219.value;
...
slop_string serialize_ttl_serialize_term(slop_arena* arena, rdf_Term t, ttl_PrefixMap prefixes) {
    slop_string _retval;
    __auto_type _mv_220 = t;
    switch (_mv_220.tag) {
        case rdf_Term_term_iri:
        {
            __auto_type iri = _mv_220.data.term_iri;
            return serialize_ttl_serialize_iri(arena, iri, prefixes);
        }
        case rdf_Term_term_blank:
        {
            __auto_type node = _mv_220.data.term_blank;
            return serialize_ttl_serialize_blank(arena, node);
        }
        case rdf_Term_term_literal:
        {
            __auto_type lit = _mv_220.data.term_literal;
...
slop_string serialize_ttl_serialize_base(slop_arena* arena, slop_option_string base) {
    __auto_type _mv_221 = base;
    if (_mv_221.has_value) {
        __auto_type iri = _mv_221.value;
...
slop_option_int serialize_ttl_find_subject_group(slop_list_serialize_ttl_SubjectGroup groups, rdf_Term subj) {
    {
        ...
        int64_t i = 0;
        {
            ...
            for (size_t _i = 0; _i < _coll.len; _i++) {
                ...
                __auto_type _mv_222 = found;
                if (_mv_222.has_value) {
                    __auto_type _ = _mv_222.value;
...
slop_option_int serialize_ttl_find_pred_group(slop_list_serialize_ttl_ObjectGroup groups, rdf_Term pred) {
    {
        ...
        int64_t i = 0;
        {
            ...
            for (size_t _i = 0; _i < _coll.len; _i++) {
                ...
                __auto_type _mv_223 = found;
                if (_mv_223.has_value) {
                    __auto_type _ = _mv_223.value;
            ...
            for (size_t _i = 0; _i < _coll.len; _i++) {
                __auto_type triple = _coll.data[_i];
                {
                    ...
                    __auto_type obj = triple.object;
                    {
                        ...
                        __auto_type _mv_224 = sg_idx;
                        if (_mv_224.has_value) {
                            __auto_type idx = _mv_224.value;
                            __auto_type _mv_225 = ({ __auto_type _lst = groups; size_t _idx = (size_t)idx; slop_option_serialize_ttl_SubjectGroup _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_225.has_value) {
                                __auto_type sg = _mv_225.value;
                                {
                                    __auto_type pg_idx = serialize_ttl_find_pred_group(sg.pred_groups, pred);
                                    __auto_type _mv_226 = pg_idx;
                                    if (_mv_226.has_value) {
                                        __auto_type pidx = _mv_226.value;
                                        __auto_type _mv_227 = ({ __auto_type _lst = sg.pred_groups; size_t _idx = (size_t)pidx; slop_option_serialize_ttl_ObjectGroup _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                        if (_mv_227.has_value) {
                                            __auto_type pg = _mv_227.value;
...
slop_result_u8_serialize_ttl_TtlFileError serialize_ttl_serialize_ttl_file(slop_arena* arena, rdf_Graph g, serialize_ttl_SerializeConfig config, slop_string path) {
    SLOP_PRE(((string_len(path) > 0)), "(> (string-len path) 0)");
    {
        __auto_type content = serialize_ttl_serialize_ttl_string(arena, g, config);
        {
            ...
            __auto_type _mv_228 = f;
            if (_mv_228.is_ok) {
                __auto_type handle = _mv_228.data.ok;
...
slop_result_u8_serialize_ttl_TtlFileError serialize_ttl_serialize_ttl_stream(slop_arena* arena, rdf_Graph g, serialize_ttl_SerializeConfig config, slop_string path) {
    ...
    {
        __auto_type f = file_file_open(path, file_FileMode_write);
        __auto_type _mv_229 = f;
        if (_mv_229.is_ok) {
            __auto_type handle = _mv_229.data.ok;
            {
