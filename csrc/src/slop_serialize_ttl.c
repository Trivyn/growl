...
slop_string serialize_ttl_serialize_iri(slop_arena* arena, rdf_IRI iri, ttl_PrefixMap prefixes) {
    ...
    slop_string _retval = {0};
    {
        ...
        __auto_type _mv_326 = compressed;
        if (_mv_326.has_value) {
            __auto_type s = _mv_326.value;
            return s;
...
slop_string serialize_ttl_serialize_literal(slop_arena* arena, rdf_Literal lit) {
    slop_string _retval = {0};
    {
        ...
        __auto_type _mv_327 = lang;
        if (_mv_327.has_value) {
            __auto_type l = _mv_327.value;
            ...
            __auto_type _mv_328 = dt;
            if (_mv_328.has_value) {
                __auto_type dt_iri = _mv_328.value;
...
slop_string serialize_ttl_serialize_term(slop_arena* arena, rdf_Term t, ttl_PrefixMap prefixes) {
    slop_string _retval = {0};
    __auto_type _mv_329 = t;
    switch (_mv_329.tag) {
        case rdf_Term_term_iri:
        {
            __auto_type iri = _mv_329.data.term_iri;
            return serialize_ttl_serialize_iri(arena, iri, prefixes);
        }
        case rdf_Term_term_blank:
        {
            __auto_type node = _mv_329.data.term_blank;
            return serialize_ttl_serialize_blank(arena, node);
        }
        case rdf_Term_term_literal:
        {
            __auto_type lit = _mv_329.data.term_literal;
...
slop_string serialize_ttl_serialize_base(slop_arena* arena, slop_option_string base) {
    __auto_type _mv_330 = base;
    if (_mv_330.has_value) {
        __auto_type iri = _mv_330.value;
...
slop_option_int serialize_ttl_find_pred_group(slop_list_serialize_ttl_ObjectGroup groups, rdf_Term pred) {
    {
        ...
        int64_t i = 0;
        {
            ...
            for (size_t _i = 0; _i < _coll.len; _i++) {
                ...
                __auto_type _mv_332 = found;
                if (_mv_332.has_value) {
                    __auto_type _ = _mv_332.value;
            ...
            for (size_t _i = 0; _i < _coll.len; _i++) {
                __auto_type triple = _coll.data[_i];
                {
                    ...
                    __auto_type obj = triple.object;
                    {
                        ...
                        __auto_type _mv_333 = sg_idx;
                        if (_mv_333.has_value) {
                            __auto_type idx = _mv_333.value;
                            __auto_type _mv_334 = ({ __auto_type _lst = groups; size_t _idx = (size_t)idx; slop_option_serialize_ttl_SubjectGroup _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_334.has_value) {
                                __auto_type sg = _mv_334.value;
                                {
                                    __auto_type pg_idx = serialize_ttl_find_pred_group(sg.pred_groups, pred);
                                    __auto_type _mv_335 = pg_idx;
                                    if (_mv_335.has_value) {
                                        __auto_type pidx = _mv_335.value;
                                        __auto_type _mv_336 = ({ __auto_type _lst = sg.pred_groups; size_t _idx = (size_t)pidx; slop_option_serialize_ttl_ObjectGroup _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                        if (_mv_336.has_value) {
                                            __auto_type pg = _mv_336.value;
...
slop_result_u8_serialize_ttl_TtlFileError serialize_ttl_serialize_ttl_stream(slop_arena* arena, rdf_Graph g, serialize_ttl_SerializeConfig config, slop_string path) {
    ...
    {
        __auto_type f = file_file_open(path, file_FileMode_write);
        __auto_type _mv_338 = f;
        if (_mv_338.is_ok) {
            __auto_type handle = _mv_338.data.ok;
            {
