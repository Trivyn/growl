    ...
    slop_string _retval = {0};
    {
        ...
        slop_option_string compressed = (slop_option_string){.has_value = false};
        {
            ...
            for (size_t _i = 0; _i < _coll.len; _i++) {
                ...
                __auto_type _mv_335 = compressed;
                if (_mv_335.has_value) {
                    __auto_type _ = _mv_335.value;
        ...
        __auto_type _mv_336 = compressed;
...
slop_string serialize_ttl_serialize_literal(slop_arena* arena, rdf_Literal lit) {
    slop_string _retval = {0};
    {
        ...
        __auto_type _mv_337 = lang;
        if (_mv_337.has_value) {
            __auto_type l = _mv_337.value;
            ...
            __auto_type _mv_338 = dt;
            if (_mv_338.has_value) {
                __auto_type dt_iri = _mv_338.value;
...
slop_string serialize_ttl_serialize_term(slop_arena* arena, rdf_Term t, ttl_PrefixMap prefixes) {
    slop_string _retval = {0};
    __auto_type _mv_339 = t;
    switch (_mv_339.tag) {
        case rdf_Term_term_iri:
        {
            __auto_type iri = _mv_339.data.term_iri;
            return serialize_ttl_serialize_iri(arena, iri, prefixes);
        }
        case rdf_Term_term_blank:
        {
            __auto_type node = _mv_339.data.term_blank;
            return serialize_ttl_serialize_blank(arena, node);
        }
        case rdf_Term_term_literal:
        {
            __auto_type lit = _mv_339.data.term_literal;
...
slop_string serialize_ttl_serialize_base(slop_arena* arena, slop_option_string base) {
    __auto_type _mv_340 = base;
    if (_mv_340.has_value) {
        __auto_type iri = _mv_340.value;
            ...
            for (size_t _i = 0; _i < _coll.len; _i++) {
                __auto_type triple = _coll.data[_i];
                {
                    ...
                    __auto_type obj = triple.object;
                    {
                        ...
                        __auto_type _mv_343 = sg_idx;
                        if (_mv_343.has_value) {
                            __auto_type idx = _mv_343.value;
                            __auto_type _mv_344 = ({ __auto_type _lst = groups; size_t _idx = (size_t)idx; slop_option_serialize_ttl_SubjectGroup _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_344.has_value) {
                                __auto_type sg = _mv_344.value;
                                {
                                    __auto_type pg_idx = serialize_ttl_find_pred_group(sg.pred_groups, pred);
                                    __auto_type _mv_345 = pg_idx;
                                    if (_mv_345.has_value) {
                                        __auto_type pidx = _mv_345.value;
                                        __auto_type _mv_346 = ({ __auto_type _lst = sg.pred_groups; size_t _idx = (size_t)pidx; slop_option_serialize_ttl_ObjectGroup _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                        if (_mv_346.has_value) {
                                            __auto_type pg = _mv_346.value;
...
slop_result_u8_serialize_ttl_TtlFileError serialize_ttl_serialize_ttl_file(slop_arena* arena, rdf_Graph g, serialize_ttl_SerializeConfig config, slop_string path) {
    SLOP_PRE(((string_len(path) > 0)), "(> (string-len path) 0)");
    {
        __auto_type content = serialize_ttl_serialize_ttl_string(arena, g, config);
        {
            ...
            __auto_type _mv_347 = f;
            if (_mv_347.is_ok) {
                __auto_type handle = _mv_347.data.ok;
                ...
                __auto_type e = _mv_347.data.err;
...
slop_result_u8_serialize_ttl_TtlFileError serialize_ttl_serialize_ttl_stream(slop_arena* arena, rdf_Graph g, serialize_ttl_SerializeConfig config, slop_string path) {
    ...
    {
        __auto_type f = file_file_open(path, file_FileMode_write);
        __auto_type _mv_348 = f;
        if (_mv_348.is_ok) {
            __auto_type handle = _mv_348.data.ok;
            {
            ...
            __auto_type e = _mv_348.data.err;
            return ((slop_result_u8_serialize_ttl_TtlFileError){ .is_ok = false, .data.err = ((serialize_ttl_TtlFileError){ .tag = serialize_ttl_TtlFileError_file_error, .data.file_error = e }) });
        }
    }
}
