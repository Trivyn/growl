...
uint8_t strlib_contains(slop_string haystack, slop_string needle) {
    __auto_type _mv_42 = strlib_index_of(haystack, needle);
    if (_mv_42.has_value) {
...
slop_string strlib_join(slop_arena* arena, slop_list_string strings, slop_string separator) {
    {
        ...
        if ((count == 0)) {
            ...
            if ((count == 1)) {
                __auto_type _mv_43 = ({ __auto_type _lst = strings; size_t _idx = (size_t)0; slop_option_string _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                if (_mv_43.has_value) {
                    __auto_type first_str = _mv_43.value;
                    return first_str;
            ...
            } else {
                {
                    ...
                    while ((i < count)) {
                        __auto_type _mv_44 = ({ __auto_type _lst = strings; size_t _idx = (size_t)i; slop_option_string _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                        if (_mv_44.has_value) {
                            __auto_type str = _mv_44.value;
                    ...
                    total_len = (total_len + (sep_len * (count - 1)));
                    {
                        ...
                        int64_t j = 0;
                        while ((j < count)) {
                            __auto_type _mv_45 = ({ __auto_type _lst = strings; size_t _idx = (size_t)j; slop_option_string _r; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                            if (_mv_45.has_value) {
                                __auto_type str = _mv_45.value;
                                if ((str.len > 0)) {
...
slop_string strlib_replace(slop_arena* arena, slop_string s, slop_string old, slop_string new) {
    __auto_type _mv_46 = strlib_index_of(s, old);
    if (!_mv_46.has_value) {
        return s;
    } else if (_mv_46.has_value) {
        __auto_type idx = _mv_46.value;
        {
            __auto_type slen = ((int64_t)(s.len));
