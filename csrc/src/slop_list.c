        ...
        while (!(rdf_term_eq(current, nil_term))) {
            {
                ...
                if (((int64_t)((first_matches.triples).len)) > 0) {
                    __auto_type _mv_102 = ({ __auto_type _lst = first_matches.triples; size_t _idx = (size_t)0; slop_option_rdf_Triple _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_102.has_value) {
                        __auto_type first_triple = _mv_102.value;
            ...
            }
            {
                ...
                if (((int64_t)((rest_matches.triples).len)) > 0) {
                    __auto_type _mv_103 = ({ __auto_type _lst = rest_matches.triples; size_t _idx = (size_t)0; slop_option_rdf_Triple _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_103.has_value) {
                        __auto_type rest_triple = _mv_103.value;
        ...
        while (!(rdf_term_eq(current, nil_term))) {
            {
                ...
                if (((int64_t)((first_matches).len)) > 0) {
                    __auto_type _mv_104 = ({ __auto_type _lst = first_matches; size_t _idx = (size_t)0; slop_option_rdf_Triple _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_104.has_value) {
                        __auto_type first_triple = _mv_104.value;
                        ({ __auto_type _lst_p = &(result); __auto_type _item = (rdf_triple_object(first_triple)); if (_lst_p->len >= _lst_p->cap) { size_t _new_cap = _lst_p->cap == 0 ? 16 : _lst_p->cap * 2; __typeof__(_lst_p->data) _new_data = (__typeof__(_lst_p->data))slop_arena_alloc(arena, _new_cap * sizeof(*_lst_p->data)); if (_lst_p->len > 0) memcpy(_new_data, _lst_p->data, _lst_p->len * sizeof(*_lst_p->data)); _lst_p->data = _new_data; _lst_p->cap = _new_cap; } _lst_p->data[_lst_p->len++] = _item; (void)0; });
            ...
            }
            {
                ...
                if (((int64_t)((rest_matches).len)) > 0) {
                    __auto_type _mv_105 = ({ __auto_type _lst = rest_matches; size_t _idx = (size_t)0; slop_option_rdf_Triple _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                    if (_mv_105.has_value) {
                        __auto_type rest_triple = _mv_105.value;
                        current = rdf_triple_object(rest_triple);
