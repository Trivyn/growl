        ...
        while (i < argc) {
            {
                ...
                if (string_eq(arg, SLOP_STR("--help")) || string_eq(arg, SLOP_STR("-h"))) {
                    ...
                    if ((i + 1) < argc) {
                        __auto_type _mv_336 = strlib_parse_int(main_argv_to_string(argv, (i + 1)));
                        if (_mv_336.is_ok) {
                            __auto_type val = _mv_336.data.ok;
                            max_tri = ((int64_t)(val));
    ...
    uint8_t** argv = (uint8_t**)_c_argv;
    {
        ...
        slop_arena* arena = &_arena;
        {
            ...
            if (args.show_version) {
                ...
                __auto_type _mv_337 = args.input_file;
                if (!_mv_337.has_value) {
                    ...
                    __auto_type input_path = _mv_337.value;
                    if (args.show_help) {
                        main_print_usage();
                        return 0;
                    } else {
                        {
                            ...
                            __auto_type _mv_338 = ttl_parse_ttl_file(arena, input_path);
                            if (!_mv_338.is_ok) {
                                __auto_type e = _mv_338.data.err;
                                printf("%s", "Error: failed to parse ");
                                printf("%.*s\n", (int)(input_path).len, (input_path).data);
                                __auto_type _mv_339 = e;
                                switch (_mv_339.tag) {
                                    case ttl_TtlFileError_parse_error:
                                    {
                                        __auto_type pe = _mv_339.data.parse_error;
                                        printf("%s", "  at line ");
                                    ...
                                    {
                                        __auto_type _ = _mv_339.data.file_error;
                                        printf("%s\n", "  (file error)");
                                        break;
                                    }
                                }
                                ...
                                __auto_type g = _mv_338.data.ok;
                                {
                                    ...
                                    }
                                    {
                                        ...
                                        __auto_type _mv_340 = args.background_file;
                                        if (_mv_340.has_value) {
                                            __auto_type bg_path = _mv_340.value;
                                            __auto_type _mv_341 = ttl_parse_ttl_file(arena, bg_path);
                                            if (!_mv_341.is_ok) {
                                                ...
                                                __auto_type bg_graph = _mv_341.data.ok;
                                                {
                                        ...
                                        }
                                        {
                                            ...
                                            }
                                            {
                                                ...
                                                __auto_type _mv_342 = growl_reason_with_config(arena, combined_ig, config);
                                                switch (_mv_342.tag) {
                                                    case types_ReasonerResult_reason_success:
                                                    {
                                                        __auto_type s = _mv_342.data.reason_success;
                                                        {
                                                            ...
                                                            if (validate_mode) {
                                                                ...
                                                                __auto_type _mv_343 = args.emit_file;
                                                                if (_mv_343.has_value) {
                                                                    __auto_type emit_path = _mv_343.value;
                                                                    {
                                                                        ...
                                                                        }
                                                                        {
                                                                            ...
                                                                            __auto_type _mv_344 = serialize_ttl_serialize_ttl_stream(arena, out_graph, config, emit_path);
                                                                            if (_mv_344.is_ok) {
                                                                                __auto_type _ = _mv_344.data.ok;
                                                                                if (!(quiet)) {
                                                    ...
                                                    {
                                                        __auto_type reports = _mv_342.data.reason_inconsistent;
                                                        {
                                                            __auto_type reason_elapsed = (slop_now_ms() - reason_start);
                                                            if (validate_mode) {
                                                                {
                                                                ...
                                                                __auto_type _mv_345 = ({ __auto_type _lst = reports; size_t _idx = (size_t)0; slop_option_types_InconsistencyReport _r = {0}; if (_idx < _lst.len) { _r.has_value = true; _r.value = _lst.data[_idx]; } else { _r.has_value = false; } _r; });
                                                                if (_mv_345.has_value) {
                                                                    __auto_type report = _mv_345.value;
                                                                    if (quiet) {
                                                                        printf("%.*s", (int)(input_path).len, (input_path).data);
                                                                        printf("%s", ": ");
                                                                        main_print_elapsed(arena, parse_elapsed);
                                                    ...
                                                    {
                                                        __auto_type s = _mv_342.data.reason_cancelled;
